 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_report_responses_from_steps_testcase extends mod_ddtaquiz_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $ddtaquizsettings settings to override default settings for ddtaquiz created by generator. Taken from ddtaquizzes.csv.
    ...
    public function test_walkthrough_from_csv($ddtaquizsettings, $csvdata) {
        ...
        $ddtaquizattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($ddtaquizattemptids[$responses['ddtaquizattempt']])) {
                throw new coding_exception("There is no ddtaquizattempt {$responses['ddtaquizattempt']}!");
            }
            $this->assert_response_test($ddtaquizattemptids[$responses['ddtaquizattempt']], $responses);
        }
    }

    protected function assert_response_test($ddtaquizattemptid, $responses) {
        $ddtaquizattempt = ddtaquiz_attempt::create($ddtaquizattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $ddtaquizattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in ddtaquizattempt {$responses['ddtaquizattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  ddtaquizattempt {$responses['ddtaquizattempt']} in $column, slot $slot, ".
