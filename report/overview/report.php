...
/**
 * This file defines the ddtaquiz overview report class.
 *
 * @package   ddtaquiz_overview
...
require_once($CFG->dirroot . '/mod/ddtaquiz/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/ddtaquiz/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/ddtaquiz/report/overview/overview_table.php');


/**
 * Ddtaquiz report subclass for the overview (grades) report.
 *
 * @copyright 1999 onwards Martin Dougiamas and others {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_overview_report extends ddtaquiz_attempts_report {

    public function display($ddtaquiz, $cm, $course) {
        global $DB, $OUTPUT, $PAGE;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'ddtaquiz_overview_settings_form', $ddtaquiz, $cm, $course);

        $options = new ddtaquiz_overview_options('overview', $ddtaquiz, $cm, $course);
        ...
        $table = new ddtaquiz_overview_table($ddtaquiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = ddtaquiz_report_download_filename(get_string('overviewfilename', 'ddtaquiz_overview'),
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/ddtaquiz:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $ddtaquiz, $groupstudentsjoins);
                    if ($currentgroup) {
                        $a= new stdClass();
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'ddtaquiz_overview', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'ddtaquiz_overview', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'ddtaquiz_overview', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'ddtaquiz_overview', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'ddtaquiz_overview');
                        $regradealllabel =
                                get_string('regradeall', 'ddtaquiz_overview');
                    }
            ...
            $this->add_grade_columns($ddtaquiz, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/ddtaquiz:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'ddtaquiz_overview');
            }
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header = get_string('qbrief', 'ddtaquiz', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . ddtaquiz_rescale_grade($question->maxmark, $ddtaquiz, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($ddtaquiz);
            $labels = self::get_bands_labels($bands, $bandwidth, $ddtaquiz);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.ddtaquiz = $ddtaquiz->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = ddtaquiz_report_grade_bands($bandwidth, $bands, $ddtaquiz->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $graphname = get_string('overviewreportgraphgroup', 'ddtaquiz_overview', groups_get_group_name($currentgroup));
                    echo $output->chart($chart, $graphname);
                }
            }

            if ($DB->record_exists('ddtaquiz_grades', array('ddtaquiz'=> $ddtaquiz->id))) {
                $data = ddtaquiz_report_grade_bands($bandwidth, $bands, $ddtaquiz->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'ddtaquiz_overview');
                echo $output->chart($chart, $graphname);
            }
        }
        return true;
     ...
     */
    protected function process_actions($ddtaquiz, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $allowedjoins, $redirecturl) {
        parent::process_actions($ddtaquiz, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($ddtaquiz, $cm);
                    $this->regrade_attempts($ddtaquiz, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($ddtaquiz, $cm);
            $this->regrade_attempts($ddtaquiz, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($ddtaquiz, $cm);
            $this->regrade_attempts($ddtaquiz, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($ddtaquiz, $cm);
            $this->regrade_attempts_needing_it($ddtaquiz, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     * @param object $cm the cm object for the ddtaquiz.
     */
    protected function start_regrade($ddtaquiz, $cm) {
        require_capability('mod/ddtaquiz:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $ddtaquiz, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'ddtaquiz_overview'));
    ...
    /**
     * Regrade a particular ddtaquiz attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the ddtaquiz_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the ddtaquiz attempt to regrade.
    ...
    protected function regrade_attempt($attempt, $dryrun = false, $slots = null) {
        global $DB;
        // Need more time for a ddtaquiz with many questions.
        ...
        $finished = $attempt->state == ddtaquiz_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('ddtaquiz_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this ddtaquiz, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($ddtaquiz, $dryrun = false,
            \core\dml\sql_join$groupstudentsjoins = null, $attemptids = array()) {
        global $DB;
        $this->unlock_session();

        $sql = "SELECT ddtaquiza.*
                  FROM {ddtaquiz_attempts} ddtaquiza";
        $where = "ddtaquiz = :qid AND preview = 0";
        $params = array('qid' => $ddtaquiz->id);
        ...
        if ($attemptids) {
            ...
            $where .= " AND ddtaquiza.id IN ({$aids})";
        }
        ...
        $this->clear_regrade_table($ddtaquiz, $groupstudentsjoins);

        $progressbar = new progress_bar('ddtaquiz_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'ddtaquiz_overview', $a));
        }

        if (!$dryrun) {
            $this->update_overall_grades($ddtaquiz);
        }
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the ddtaquiz_overview_regrades table.
     * @param object $ddtaquiz the ddtaquiz settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($ddtaquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{ddtaquiz_overview_regrades} qqr ON qqr.questionusageid = ddtaquiza.uniqueid';
        $where = "ddtaquiza.ddtaquiz = :qid AND ddtaquiza.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $ddtaquiz->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT ddtaquiza.uniqueid, qqr.slot
                  FROM {ddtaquiz_attempts} ddtaquiza
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_list('ddtaquiz_attempts', 'uniqueid',
     ...
     */
    protected function count_question_attempts_needing_regrade($ddtaquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = ddtaquiza.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = ddtaquiza.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['cddtaquiz'] = $ddtaquiz->id;
        $sql = "SELECT COUNT(DISTINCT ddtaquiza.id)
                  FROM {ddtaquiz_attempts} ddtaquiza
                  JOIN {ddtaquiz_overview_regrades} qqr ON ddtaquiza.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      ddtaquiza.ddtaquiz = :cddtaquiz AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {ddtaquiz_overview_regrades} qor ON qor.questionusageid = ddtaquiza.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($ddtaquiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {ddtaquiz_attempts} ddtaquiza";
        $where = "WHERE ddtaquiza.ddtaquiz = :qid";
        $params = array('qid' => $ddtaquiz->id);
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = ddtaquiza.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('ddtaquiz_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($ddtaquiz) {
    ...
    /**
     * Get the bands configuration for the ddtaquiz.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a ddtaquiz. The width of
     ...
     */
    public static function get_bands_count_and_width($ddtaquiz) {
        $bands = $ddtaquiz->grade;
        ...
        return [$bands, $ddtaquiz->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $ddtaquiz) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = ddtaquiz_format_grade($ddtaquiz, ($i - 1) * $bandwidth) . ' - ' . ddtaquiz_format_grade($ddtaquiz, $i * $bandwidth);
        }
        return $bandlabels;
    }
