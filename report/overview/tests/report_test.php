...
/**
 * Tests for the ddtaquiz overview report.
 *
...
require_once($CFG->dirroot . '/mod/ddtaquiz/report/overview/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql($isdownloading) {
        ...
        // Create a course and a ddtaquiz.
        $generator = $this->getDataGenerator();
        $course = $generator->create_course();
        $ddtaquizgenerator = $generator->get_plugin_generator('mod_ddtaquiz');
        $ddtaquiz = $ddtaquizgenerator->create_instance(array('course' => $course->id,
                'grademethod' => DDTAQUIZ_GRADEHIGHEST, 'grade' => 100.0, 'sumgrades' => 10.0,
                'attempts' => 10));
        ...
        ddtaquiz_add_ddtaquiz_question($q->id, $ddtaquiz, 0 , 10);
        ...
        $attempts = array(
            array($ddtaquiz, $student1, 1, 0.0,  ddtaquiz_attempt::FINISHED),
            array($ddtaquiz, $student1, 2, 5.0,  ddtaquiz_attempt::FINISHED),
            array($ddtaquiz, $student1, 3, 8.0,  ddtaquiz_attempt::FINISHED),
            array($ddtaquiz, $student1, 4, null, ddtaquiz_attempt::ABANDONED),
            array($ddtaquiz, $student1, 5, null, ddtaquiz_attempt::IN_PROGRESS),
            array($ddtaquiz, $student2, 1, null, ddtaquiz_attempt::ABANDONED),
            array($ddtaquiz, $student2, 2, null, ddtaquiz_attempt::ABANDONED),
            array($ddtaquiz, $student2, 3, 7.0,  ddtaquiz_attempt::FINISHED),
            array($ddtaquiz, $student2, 4, null, ddtaquiz_attempt::ABANDONED),
            array($ddtaquiz, $student2, 5, null, ddtaquiz_attempt::ABANDONED),
        );

        // Load it in to ddtaquiz attempts table.
        foreach ($attempts as $attemptdata) {
            list($ddtaquiz, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = ddtaquiz_create_attempt($ddtaquizobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case ddtaquiz_attempt::FINISHED:
                    ...
                    $DB->update_record('ddtaquiz_attempts', $update);
                    ddtaquiz_save_best_grade($attemptobj->get_ddtaquiz(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = ddtaquiz_report_qm_filter_select($ddtaquiz);
        $studentsjoins = get_enrolled_with_capabilities_join($context);
        $empty = new \core\dml\sql_join();

        // Set the options.
        $reportoptions = new ddtaquiz_overview_options('overview', $ddtaquiz, $cm, null);
        $reportoptions->attempts = ddtaquiz_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(ddtaquiz_attempt::IN_PROGRESS, ddtaquiz_attempt::OVERDUE, ddtaquiz_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new ddtaquiz_overview_table($ddtaquiz, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new moodle_url('/mod/ddtaquiz/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width($grade, $expected) {
        ...
        $ddtaquiz = $ddtaquizgenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, ddtaquiz_overview_report::get_bands_count_and_width($ddtaquiz));
    }
