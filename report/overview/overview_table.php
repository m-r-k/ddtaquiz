...
require_once($CFG->dirroot . '/mod/ddtaquiz/report/attemptsreport_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_overview_table extends ddtaquiz_attempts_report_table {
     ...
     * @param ddtaquiz_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($ddtaquiz, $context, $qmsubselect,
            ddtaquiz_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-ddtaquiz-report-overview-report', $ddtaquiz , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the ddtaquiz.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(ddtaquizaouter.sumgrades) AS grade, COUNT(ddtaquizaouter.sumgrades) AS numaveraged
                  FROM {ddtaquiz_attempts} ddtaquizaouter
                  JOIN (
                       SELECT DISTINCT ddtaquiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON ddtaquizaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = ddtaquiz_rescale_grade($record->grade, $this->ddtaquiz, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(ddtaquiz_report_feedback_for_grade(
                                         $record->grade, $this->ddtaquiz->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{ddtaquiz_attempts} ddtaquizaouter
                  JOIN (
                       SELECT DISTINCT ddtaquiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON ddtaquizaouter.id = relevant_attempt_ids.id",
                    'ddtaquizaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = ddtaquiz_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->ddtaquiz, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = ddtaquiz_format_question_grade($this->ddtaquiz, $record->grade);
        } else {
            $average = ddtaquiz_format_grade($this->ddtaquiz, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/ddtaquiz:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'ddtaquiz_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = ddtaquiz_rescale_grade($attempt->sumgrades, $this->ddtaquiz);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = ddtaquiz_rescale_grade($newsumgrade, $this->ddtaquiz);
            $oldsumgrade = ddtaquiz_rescale_grade($oldsumgrade, $this->ddtaquiz);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/ddtaquiz/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'ddtaquiz')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/ddtaquiz/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = ddtaquiz_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->ddtaquiz, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = ddtaquiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->ddtaquiz, 'question');
            $oldgrade = ddtaquiz_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'ddtaquiz_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'ddtaquiz_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {ddtaquiz_overview_regrades} qqr
                                 WHERE qqr.questionusageid = ddtaquiza.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/ddtaquiz:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('ddtaquiz_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return ddtaquiz_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
