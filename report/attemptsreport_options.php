...
/**
 * Base class for the options that control what is visible in an {@link ddtaquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the ddtaquiz being reported on. */
    public $ddtaquiz;

    /** @var object the course module objects for the ddtaquiz being reported on. */
    public $cm;

    /** @var object the course settings for the course the ddtaquiz is in. */
    public $course;

    /**
     * @var array form field name => corresponding ddtaquiz_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => ddtaquiz_attempt::IN_PROGRESS,
        'stateoverdue'    => ddtaquiz_attempt::OVERDUE,
        'statefinished'   => ddtaquiz_attempt::FINISHED,
        'stateabandoned'  => ddtaquiz_attempt::ABANDONED,
    );

    /**
     * @var string ddtaquiz_attempts_report::ALL_WITH or ddtaquiz_attempts_report::ENROLLED_WITH
     *      ddtaquiz_attempts_report::ENROLLED_WITHOUT or ddtaquiz_attempts_report::ENROLLED_ALL
     */
    public $attempts = ddtaquiz_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of ddtaquiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(ddtaquiz_attempt::IN_PROGRESS, ddtaquiz_attempt::OVERDUE,
            ddtaquiz_attempt::FINISHED, ddtaquiz_attempt::ABANDONED);
    ...
    public $pagesize = ddtaquiz_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $ddtaquiz the settings for the ddtaquiz being reported on.
     * @param object $cm the course module objects for the ddtaquiz being reported on.
     * @param object $coures the course settings for the coures this ddtaquiz is in.
     */
    public function __construct($mode, $ddtaquiz, $cm, $course) {
        $this->mode   = $mode;
        $this->ddtaquiz   = $ddtaquiz;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = ddtaquiz_report_should_show_grades($ddtaquiz, context_module::instance($cm->id));
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == ddtaquiz_attempts_report::ALL_WITH) {
                $this->attempts = ddtaquiz_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = ddtaquiz_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(ddtaquiz_attempts_report::ALL_WITH, ddtaquiz_attempts_report::ENROLLED_WITH,
                ddtaquiz_attempts_report::ENROLLED_WITHOUT, ddtaquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = ddtaquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!ddtaquiz_report_can_filter_only_graded($this->ddtaquiz)) {
        ...
        if ($this->attempts == ddtaquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = ddtaquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(ddtaquiz_attempt::FINISHED, $this->states);
    }
}
