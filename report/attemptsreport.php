...
/**
 * Base class for ddtaquiz reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class ddtaquiz_attempts_report extends ddtaquiz_default_report {
    ...
    /** @var object the ddtaquiz context. */
    protected $context;

    /** @var mod_ddtaquiz_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $ddtaquiz, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = ddtaquiz_report_qm_filter_select($ddtaquiz);

        $this->form = new $formclass($this->get_base_url(),
                array('ddtaquiz' => $ddtaquiz, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/ddtaquiz:attempt', 'mod/ddtaquiz:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a ddtaquiz report.
     *
     ...
     * @param mod_ddtaquiz_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the ddtaquiz.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $ddtaquiz,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $ddtaquiz, $this->mode);
        ...
        if ($strattemptnum = ddtaquiz_num_attempt_summary($ddtaquiz, $cm, true, $currentgroup)) {
            echo '<div class="ddtaquizattemptcounts">' . $strattemptnum . '</div>';
        }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this ddtaquiz.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($ddtaquiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
        ...
        if ($includefeedback && ddtaquiz_has_feedback($ddtaquiz)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'ddtaquiz');
        }
    }
     ...
     * @param mod_ddtaquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_ddtaquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/ddtaquiz:deleteattempts', $this->context);
                    $this->delete_selected_attempts($ddtaquiz, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the ddtaquiz attempts
     * @param object $ddtaquiz the ddtaquiz settings. Attempts that don't belong to
     * this ddtaquiz are not deleted.
     ...
     */
    protected function delete_selected_attempts($ddtaquiz, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT ddtaquiza.*
                         ...
                         WHERE ddtaquiza.id = :attemptid";
            } else {
                $sql = "SELECT ddtaquiza.*
                          FROM {ddtaquiz_attempts} ddtaquiza
                          JOIN {user} u ON u.id = ddtaquiza.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND ddtaquiza.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params);
            if (!$attempt || $attempt->ddtaquiz != $ddtaquiz->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this ddtaquiz and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling ddtaquiz_delete_attempt().
            $ddtaquiz->cmid = $cm->id;
