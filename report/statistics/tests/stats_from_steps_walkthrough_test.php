 ...
 *
 * The ddtaquiz stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-ddtaquiz-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/ddtaquiz/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/ddtaquiz/report/default.php');
require_once($CFG->dirroot . '/mod/ddtaquiz/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_report_statistics_from_steps_testcase extends mod_ddtaquiz_attempt_walkthrough_from_csv_testcase {

    /**
     * @var ddtaquiz_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a ddtaquiz add questions to it, walk through ddtaquiz attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($ddtaquizsettings, $csvdata) {
        ...
        $whichattempts = DDTAQUIZ_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudentsjoins = new \core\dml\sql_join();
        list($questions, $ddtaquizstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($ddtaquizsettings['testnumber'] === '00') {
            $this->check_variants_count_for_ddtaquiz_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_ddtaquiz_stats_for_ddtaquiz_00($ddtaquizstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_ddtaquiz_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $ddtaquizstats
     */
    protected function check_ddtaquiz_stats_for_ddtaquiz_00($ddtaquizstats) {
        $ddtaquizstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($ddtaquizstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $ddtaquizstats->$statname, $ddtaquizstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $ddtaquizstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new ddtaquiz_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->ddtaquiz);
        list($ddtaquizstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->ddtaquiz,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = ddtaquiz_statistics_qubaids_condition($this->ddtaquiz->id, $groupstudentsjoins, $whichattempts);

        // We will create some ddtaquiz and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $ddtaquizcalc = new \ddtaquiz_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($ddtaquizcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $ddtaquizstats, $questionstats, $qubaids);
        }
        return array($questions, $ddtaquizstats, $questionstats, $qubaids);
    }
