...
/**
 * Standard plugin entry points of the ddtaquiz statistics report.
 *
 ...
 * @package  ddtaquiz_statistics
 * @category files
 * @param context $previewcontext the ddtaquiz context
 ...
 */
function ddtaquiz_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('ddtaquiz/statistics:view', $context);
...
/**
 * Ddtaquiz statistics report cron code. Deletes cached data more than a certain age.
 */
function ddtaquiz_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old ddtaquiz statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('ddtaquiz_statistics', 'timemodified < ?', array($expiretime));
