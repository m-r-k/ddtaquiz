...
class calculated {
    ...
    /* Following stats all described here : http://docs.moodle.org/dev/Ddtaquiz_statistics_calculations#Test_statistics  */
     ...
     */
    public function get_formatted_ddtaquiz_info_data($course, $cm, $ddtaquiz) {
        ...
        // General information about the ddtaquiz.
        $ddtaquizinfo = array();
        $ddtaquizinfo[get_string('ddtaquizname', 'ddtaquiz_statistics')] = format_string($ddtaquiz->name);
        $ddtaquizinfo[get_string('coursename', 'ddtaquiz_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $ddtaquizinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($ddtaquiz->timeopen) {
            $ddtaquizinfo[get_string('ddtaquizopen', 'ddtaquiz')] = userdate($ddtaquiz->timeopen);
        }
        if ($ddtaquiz->timeclose) {
            $ddtaquizinfo[get_string('ddtaquizclose', 'ddtaquiz')] = userdate($ddtaquiz->timeclose);
        }
        if ($ddtaquiz->timeopen && $ddtaquiz->timeclose) {
            $ddtaquizinfo[get_string('duration', 'ddtaquiz_statistics')] =
                format_time($ddtaquiz->timeclose - $ddtaquiz->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = ddtaquiz_report_scale_summarks_as_percentage($value, $ddtaquiz);
                    break;
                case 'number_format_percent':
                    $formattedvalue = ddtaquiz_format_grade($ddtaquiz, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $ddtaquiz->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $ddtaquizinfo[get_string($property, 'ddtaquiz_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('ddtaquiz_statistics', $toinsert);

    }

    /**
     * Given a record from 'ddtaquiz_statistics' table load the data into the properties of this class.
     *
