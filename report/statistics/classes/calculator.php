...
namespace ddtaquiz_statistics;
defined('MOODLE_INTERNAL') || die();

/**
 * Class to calculate and also manage caching of ddtaquiz statistics.
 *
 * These ddtaquiz statistics calculations are described here :
 *
 * http://docs.moodle.org/dev/Ddtaquiz_statistics_calculations#Test_statistics
 *
...
class calculator {
    ...
    /**
     * Compute the ddtaquiz statistics.
     *
     ...
     * @return calculated $ddtaquizstats The statistics for overall attempt scores.
     */
    public function calculate($ddtaquizid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $ddtaquizstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($ddtaquizid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $ddtaquizstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                ddtaquiz_statistics_attempts_sql($ddtaquizid, $groupstudentsjoins, $whichattempts);

            $ddtaquizstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($ddtaquizstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $ddtaquizstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/Ddtaquiz_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $ddtaquizstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $ddtaquizstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $ddtaquizstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $ddtaquizstats->errorratio = 100 * sqrt(1 - ($ddtaquizstats->cic / 100));
                            $ddtaquizstats->standarderror = $ddtaquizstats->errorratio *
                                $ddtaquizstats->standarddeviation / 100;
                        }
                    }
            ...
            $ddtaquizstats->cache(ddtaquiz_statistics_qubaids_condition($ddtaquizid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $ddtaquizstats;
    }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('ddtaquiz_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('ddtaquiz_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular ddtaquiz grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'ddtaquiz_statistics');
    }

    /**
     * Given a particular ddtaquiz grading method return a string id for use as a field name prefix in mdl_ddtaquiz_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/Ddtaquiz_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($ddtaquizid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(ddtaquiz_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = ddtaquiz_statistics_attempts_sql($ddtaquizid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/Ddtaquiz_statistics_calculations#Median_Score
     *
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT ddtaquiza.id, ddtaquiza.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/Ddtaquiz_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((ddtaquiza.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((ddtaquiza.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((ddtaquiza.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
