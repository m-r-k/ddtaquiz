...
/**
 * This file defines the ddtaquiz manual grading report class.
 *
 * @package   ddtaquiz_grading
...
require_once($CFG->dirroot . '/mod/ddtaquiz/report/grading/gradingsettings_form.php');


/**
 * Ddtaquiz report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_grading_report extends ddtaquiz_default_report {
    ...
    public function display($ddtaquiz, $cm, $course) {
        ...
        require_capability('mod/ddtaquiz:grade', $this->context);
        $shownames = has_capability('ddtaquiz/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('ddtaquiz/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this ddtaquiz.
        $this->questions = ddtaquiz_report_get_significant_questions($ddtaquiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'ddtaquiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/ddtaquiz:reviewmyattempts', 'mod/ddtaquiz:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'ddtaquiz_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $ddtaquiz, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "ddtaquiza.ddtaquiz = :mangrddtaquizid AND
                ddtaquiza.preview = 0 AND
                ddtaquiza.state = :statefinished";
        $params = array('mangrddtaquizid' => $this->cm->instance, 'statefinished' => ddtaquiz_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND ddtaquiza.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON ddtaquiza.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{ddtaquiz_attempts} ddtaquiza $usersjoin ", 'ddtaquiza.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = ddtaquiz_attempt::FINISHED;
        $params[] = $this->ddtaquiz->id;

        $fields = 'ddtaquiza.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {ddtaquiz_attempts} ddtaquiza
                JOIN {user} u ON u.id = ddtaquiza.userid
                WHERE ddtaquiza.uniqueid $asql AND ddtaquiza.state = ? AND ddtaquiza.ddtaquiz = ?",
                $params);
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'ddtaquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'ddtaquiz_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'ddtaquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'ddtaquiz_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'ddtaquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'ddtaquiz_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'ddtaquiz_grading');
        $table->head[] = get_string('tograde', 'ddtaquiz_grading');
        $table->head[] = get_string('alreadygraded', 'ddtaquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'ddtaquiz_grading');
        }
        $table->head[] = get_string('total', 'ddtaquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new ddtaquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'ddtaquiz_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'ddtaquiz_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'ddtaquiz_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = ddtaquiz_get_review_options($this->ddtaquiz, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'ddtaquiz_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'ddtaquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'ddtaquiz_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new ddtaquiz_attempt($attempt, $this->ddtaquiz, $this->cm, $this->course);
            ...
            $params = array(
                ...
                'other' => array(
                    'ddtaquizid' => $attemptobj->get_ddtaquizid(),
                    'attemptid' => $attemptobj->get_attemptid(),
                    'slot' => $assumedslotforevents
                )
            );
            $events[] = \mod_ddtaquiz\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON ddtaquiza.userid = u.id ";
