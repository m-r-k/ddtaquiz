...
/**
 * Base class for the table used by a {@link ddtaquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class ddtaquiz_attempts_report_table extends table_sql {
    ...
    /** @var object the ddtaquiz settings for the ddtaquiz we are reporting on. */
    protected $ddtaquiz;
    ...
    /** @var object mod_ddtaquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this ddtaquiz.. */
    protected $questions;
     ...
     * @param mod_ddtaquiz_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $ddtaquiz, $context, $qmsubselect,
            mod_ddtaquiz_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return ddtaquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = ddtaquiz_report_feedback_for_grade(
                ddtaquiz_rescale_grade($attempt->sumgrades, $this->ddtaquiz, false),
                $this->ddtaquiz->id, $this->context);
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as ddtaquiza.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(ddtaquiza.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                ddtaquiza.uniqueid AS usageid,
                ddtaquiza.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                ddtaquiza.state,
                ddtaquiza.sumgrades,
                ddtaquiza.timefinish,
                ddtaquiza.timestart,
                CASE WHEN ddtaquiza.timefinish = 0 THEN null
                     WHEN ddtaquiza.timefinish > ddtaquiza.timestart THEN ddtaquiza.timefinish - ddtaquiza.timestart
        ...
        $from = " {user} u";
        $from .= "\nLEFT JOIN {ddtaquiz_attempts} ddtaquiza ON
                                    ddtaquiza.userid = u.id AND ddtaquiza.ddtaquiz = :ddtaquizid";
        $params = array('ddtaquizid' => $this->ddtaquiz->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (ddtaquiza.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = ddtaquiz_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case ddtaquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'ddtaquiza.id IS NOT NULL AND ddtaquiza.preview = 0';
                break;
            case ddtaquiz_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "ddtaquiza.preview = 0 AND ddtaquiza.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case ddtaquiz_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "ddtaquiza.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case ddtaquiz_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(ddtaquiza.preview = 0 OR ddtaquiza.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (ddtaquiza.state $statesql OR ddtaquiza.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{ddtaquiz_attempts} {$alias}ddtaquiza", "{$alias}ddtaquiza.uniqueid",
                "{$alias}ddtaquiza.ddtaquiz = :{$alias}ddtaquizid", array("{$alias}ddtaquizid" => $this->sql->params['ddtaquizid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = ddtaquiza.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT ddtaquiza.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) ddtaquizasubquery", 'ddtaquizasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['ddtaquiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'ddtaquiz') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'ddtaquiz') . '</a> ';
