...
/**
 * Defines the Moodle forum used to add random questions to the ddtaquiz.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'ddtaquiz'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'ddtaquiz'));
        ...
        $options = array(
            'multiple' => true,
            'noselectionstring' => get_string('anytags', 'ddtaquiz'),
        );
        $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_ddtaquiz'), $tagstrings, $options);
        $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_ddtaquiz');

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'ddtaquiz'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_ddtaquiz/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'ddtaquiz'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'ddtaquiz'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'ddtaquiz'));
        ...
        $PAGE->requires->js_call_amd('mod_ddtaquiz/add_random_form', 'init', [
