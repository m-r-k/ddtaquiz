...
/**
 * Back-end code for handling data about ddtaquizzes and the current user's attempt.
 *
 * There are classes for loading all the information about a ddtaquiz and attempts,
...
/**
 * Class for ddtaquiz exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_ddtaquiz_exception extends moodle_exception {
    public function __construct($ddtaquizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $ddtaquizobj->view_url();
        }
        parent::__construct($errorcode, 'ddtaquiz', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a ddtaquiz and the questions it contains, and making the
...
class ddtaquiz {
    ...
    protected $cm;
    /** @var stdClass the ddtaquiz settings from the database. */
    protected $ddtaquiz;
    /** @var context the ddtaquiz context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of ddtaquiz_section rows. */
    protected $sections = null;
    /** @var ddtaquiz_access_manager the access manager for this ddtaquiz. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/ddtaquiz:preview. */
     ...
     *
     * @param object $ddtaquiz the row from the ddtaquiz table.
     * @param object $cm the course_module object for this ddtaquiz.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($ddtaquiz, $cm, $course, $getcontext = true) {
        $this->ddtaquiz = $ddtaquiz;
        $this->cm = $cm;
        $this->ddtaquiz->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new ddtaquiz object for a specific user.
     *
     * @param int $ddtaquizid the the ddtaquiz id.
     * @param int $userid the the userid.
     * @return ddtaquiz the new ddtaquiz object
     */
    public static function create($ddtaquizid, $userid = null) {
        global $DB;

        $ddtaquiz = ddtaquiz_access_manager::load_ddtaquiz_and_settings($ddtaquizid);
        ...
        if ($userid) {
            $ddtaquiz = ddtaquiz_update_effective_access($ddtaquiz, $userid);
        }

        return new ddtaquiz($ddtaquiz, $cm, $course);
    }

    /**
     * Create a {@link ddtaquiz_attempt} for an attempt at this ddtaquiz.
     * @param object $attemptdata row from the ddtaquiz_attempts table.
     * @return ddtaquiz_attempt the new ddtaquiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new ddtaquiz_attempt($attemptdata, $this->ddtaquiz, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this ddtaquiz.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{ddtaquiz_slots} slot ON slot.ddtaquizid = :ddtaquizid AND q.id = slot.questionid',
                array('ddtaquizid' => $this->ddtaquiz->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this ddtaquiz. You must call
    ...
    /**
     * Get an instance of the {@link \mod_ddtaquiz\structure} class for this ddtaquiz.
     * @return \mod_ddtaquiz\structure describes the questions in the ddtaquiz.
     */
    public function get_structure() {
        return \mod_ddtaquiz\structure::create_for_ddtaquiz($this);
    }
    ...
    /** @return int the ddtaquiz id. */
    public function get_ddtaquizid() {
        return $this->ddtaquiz->id;
    }

    /** @return object the row of the ddtaquiz table. */
    public function get_ddtaquiz() {
        return $this->ddtaquiz;
    }

    /** @return string the name of this ddtaquiz. */
    public function get_ddtaquiz_name() {
        return $this->ddtaquiz->name;
    }

    /** @return int the ddtaquiz navigation method. */
    public function get_navigation_method() {
        return $this->ddtaquiz->navmethod;
    }

    /** @return int the number of attempts allowed at this ddtaquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->ddtaquiz->attempts;
    }
    ...
    /** @return object the module context for this ddtaquiz. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the ddtaquiz,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/ddtaquiz:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this ddtaquiz.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'ddtaquiz', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this ddtaquiz.
     * @return array 0, 1, 2, ... => ddtaquiz_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('ddtaquiz_sections',
                    array('ddtaquizid' => $this->get_ddtaquizid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return ddtaquiz_access_manager and instance of the ddtaquiz_access_manager class
     * for this ddtaquiz at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new ddtaquiz_access_manager($this, $timenow,
                    has_capability('mod/ddtaquiz:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the ddtaquiz context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the ddtaquiz context.
     */
    ...
    /**
     * @return string the URL of this ddtaquiz's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/ddtaquiz/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this ddtaquiz's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/ddtaquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/ddtaquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this ddtaquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/ddtaquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/ddtaquiz/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/ddtaquiz/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by ddtaquiz access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by ddtaquiz access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_ddtaquiz_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_ddtaquiz_display_options::DURING ||
                $when == mod_ddtaquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_ddtaquiz_display_options::LATER_WHILE_OPEN && $this->ddtaquiz->timeclose &&
                $this->ddtaquiz->reviewattempt & mod_ddtaquiz_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'ddtaquiz',
                    userdate($this->ddtaquiz->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'ddtaquiz');
        }
    }

    /**
     * @param string $title the name of this particular ddtaquiz page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_ddtaquiz_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this ddtaquiz.
     *
     * @param  boolean $includepotential if the ddtaquiz include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the ddtaquiz may content
...
/**
 * This class extends the ddtaquiz class to hold data about the state of a particular attempt,
 * in addition to the data about the ddtaquiz.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class ddtaquiz_attempt {
    ...
    /** @var int maximum number of slots in the ddtaquiz for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var ddtaquiz object containing the ddtaquiz settings. */
    protected $ddtaquizobj;

    /** @var stdClass the ddtaquiz_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this ddtaquiz attempt. */
    protected $quba;
    ...
    /** @var array of ddtaquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_ddtaquiz_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the ddtaquiz_attempts table.
     * @param object $ddtaquiz the ddtaquiz object for this attempt and user.
     * @param object $cm the course_module object for this ddtaquiz.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $ddtaquiz, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->ddtaquizobj = new ddtaquiz($ddtaquiz, $cm, $course);
        ...
        $this->slots = $DB->get_records('ddtaquiz_slots',
                array('ddtaquizid' => $this->get_ddtaquizid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('ddtaquiz_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('ddtaquiz_attempts', $conditions, '*', MUST_EXIST);
        $ddtaquiz = ddtaquiz_access_manager::load_ddtaquiz_and_settings($attempt->ddtaquiz);
        $course = $DB->get_record('course', array('id' => $ddtaquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('ddtaquiz', $ddtaquiz->id, $course->id, false, MUST_EXIST);

        // Update ddtaquiz with override information.
        $ddtaquiz = ddtaquiz_update_effective_access($ddtaquiz, $attempt->userid);

        return new ddtaquiz_attempt($attempt, $ddtaquiz, $cm, $course);
    }

    /**
     * Static function to create a new ddtaquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new ddtaquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return ddtaquiz_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'ddtaquiz');
                }
    ...
    // Simple getters ==========================================================
    public function get_ddtaquiz() {
        return $this->ddtaquizobj->get_ddtaquiz();
    }

    public function get_ddtaquizobj() {
        return $this->ddtaquizobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->ddtaquizobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->ddtaquizobj->get_course();
    }

    /** @return int the ddtaquiz id. */
    public function get_ddtaquizid() {
        return $this->ddtaquizobj->get_ddtaquizid();
    }

    /** @return string the name of this ddtaquiz. */
    public function get_ddtaquiz_name() {
        return $this->ddtaquizobj->get_ddtaquiz_name();
    }

    /** @return int the ddtaquiz navigation method. */
    public function get_navigation_method() {
        return $this->ddtaquizobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->ddtaquizobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->ddtaquizobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->ddtaquizobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this ddtaquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->ddtaquizobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this ddtaquiz. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->ddtaquizobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the ddtaquiz_attempts table. */
    ...
    /** @return string one of the ddtaquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/ddtaquiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the ddtaquiz in a non-trivial way?
     ...
     * To see how this works, try setting a ddtaquiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the ddtaquiz
     ...
     *
     * @param question_display_options $options the display options for this ddtaquiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return ddtaquiz_feedback_for_grade($grade, $this->get_ddtaquiz(),
                $this->ddtaquizobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->ddtaquizobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->ddtaquizobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_ddtaquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/ddtaquiz:attempt';
        } else {
            $capability = 'mod/ddtaquiz:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a ddtaquiz
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the ddtaquiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/ddtaquiz:viewreports') ||
                $this->has_capability('mod/ddtaquiz:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case DDTAQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case DDTAQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return ddtaquiz_attempt_state($this->get_ddtaquiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_ddtaquiz_display_options for this ddtaquiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = ddtaquiz_get_review_options($this->get_ddtaquiz(),
                        $this->attempt, $this->ddtaquizobj->get_context());
            ...
            $options = mod_ddtaquiz_display_options::make_from_ddtaquiz($this->get_ddtaquiz(),
                    mod_ddtaquiz_display_options::DURING);
            $options->flags = ddtaquiz_get_flag_option($this->attempt, $this->ddtaquizobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the ddtaquiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the ddtaquiz, or for the
     * whole ddtaquiz.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the ddtaquiz_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != DDTAQUIZ_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_ddtaquiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the ddtaquiz where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the ddtaquiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the ddtaquiz.
     */
    public function get_question_mark($slot) {
        return ddtaquiz_format_question_grade($this->get_ddtaquiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->ddtaquizobj->get_ddtaquiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->ddtaquizobj->get_ddtaquiz()->timelimit;
        }
        if ($this->ddtaquizobj->get_ddtaquiz()->timeclose) {
            $deadlines[] = $this->ddtaquizobj->get_ddtaquiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->ddtaquizobj->get_ddtaquiz()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string ddtaquiz view url.
     */
    public function view_url() {
        return $this->ddtaquizobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->ddtaquizobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this ddtaquiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/ddtaquiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/ddtaquiz/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->ddtaquizobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'ddtaquiz'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_ddtaquiz_renderer $renderer the ddtaquiz renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_ddtaquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_ddtaquiz-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_ddtaquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'ddtaquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'ddtaquiz');
        ...
        $placeholderqa->start($this->get_ddtaquiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this ddtaquiz attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_ddtaquiz_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, ddtaquiz_attempt_nav_panel or ddtaquiz_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole ddtaquiz on one page. (Used by review.php)
     * @return ddtaquiz_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_ddtaquiz_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_ddtaquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_ddtaquiz_navblock_title';
        $bc->title = html_writer::span(get_string('ddtaquiznavigation', 'ddtaquiz'), '', array('id' => 'mod_ddtaquiz_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this ddtaquiz.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_ddtaquiz_links_to_other_attempts} object returned contains an
     ...
     * @return mod_ddtaquiz_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = ddtaquiz_get_user_attempts($this->get_ddtaquiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_ddtaquiz_links_to_other_attempts();
     ...
     * @return mod_ddtaquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->ddtaquizobj->get_ddtaquiz()->graceperiod;
        ...
        // Otherwise, we were in ddtaquiz_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->ddtaquizobj->get_ddtaquiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('ddtaquiz_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            ddtaquiz_save_best_grade($this->get_ddtaquiz(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_ddtaquiz\question\qubaids_for_users_attempts(
                $this->get_ddtaquizid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = ddtaquiz_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'ddtaquiz',
                        $this->ddtaquizobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            ddtaquiz_save_best_grade($this->get_ddtaquiz(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_ddtaquiz\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('ddtaquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_ddtaquiz\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        ddtaquiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_ddtaquiz\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $ddtaquizrecord = $this->get_ddtaquiz();
        $params = array(
            'context' => $this->get_ddtaquizobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'ddtaquizid' => $ddtaquizrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('ddtaquiz', $this->get_ddtaquiz());
        $event->add_record_snapshot('ddtaquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the ddtaquiz.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/ddtaquiz/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/ddtaquiz/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a ddtaquiz.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the ddtaquiz. Just finish now.
        $graceperiodmin = null;
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - DDTAQUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_ddtaquiz()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('ddtaquiz', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_ddtaquiz()->graceperiod + $graceperiodmin) {
        ...
        // Update the ddtaquiz attempt record.
        try {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == DDTAQUIZ_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('ddtaquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'ddtaquizid' => $this->get_ddtaquizid()
            )
        );
        $event = \mod_ddtaquiz\event\attempt_viewed::create($params);
        $event->add_record_snapshot('ddtaquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    public function fire_attempt_summary_viewed_event() {
        ...
        $event = \mod_ddtaquiz\event\attempt_summary_viewed::create($params);
        $event->add_record_snapshot('ddtaquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
 ...
 * @since      Moodle 2.9
 */
class ddtaquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class ddtaquiz_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class ddtaquiz_nav_panel_base {
    /** @var ddtaquiz_attempt */
    ...
    public function __construct(ddtaquiz_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the ddtaquiz navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new ddtaquiz_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new ddtaquiz_nav_question_button();
            $button->id          = 'ddtaquiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'ddtaquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'ddtaquiz');
        } else {
            return get_string('viewed', 'ddtaquiz');
        }
    }

    public function render_before_button_bits(mod_ddtaquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_ddtaquiz_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_ddtaquiz()->showuserpicture == DDTAQUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_ddtaquiz()->showuserpicture == DDTAQUIZ_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // Ddtaquiz navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // Ddtaquiz navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link ddtaquiz_nav_panel_base} for the attempt ddtaquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class ddtaquiz_attempt_nav_panel extends ddtaquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_ddtaquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'ddtaquiz'),
                array('id' => 'ddtaquiznojswarning'));
    }

    public function render_end_bits(mod_ddtaquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'ddtaquiz'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link ddtaquiz_nav_panel_base} for the review ddtaquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class ddtaquiz_review_nav_panel extends ddtaquiz_nav_panel_base {
    public function get_question_url($slot) {
