...
/**
 * This page handles editing and creation of ddtaquiz overrides
 *
...
$url = new moodle_url('/mod/ddtaquiz/overrideedit.php');
if ($action) {
...
// Merge ddtaquiz defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $ddtaquiz->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/ddtaquiz/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new ddtaquiz_override_form($url, $cm, $ddtaquiz, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->ddtaquiz = $ddtaquiz->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $ddtaquiz->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('ddtaquiz_overrides', $conditions)) {
            ...
            $ddtaquiz->cmid = $cm->id;
            ddtaquiz_delete_override($ddtaquiz, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('ddtaquiz_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('ddtaquiz_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        ddtaquiz_update_events($ddtaquiz, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'ddtaquiz');
