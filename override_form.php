...
/**
 * Settings form for overrides in the ddtaquiz module.
 *
...
require_once($CFG->dirroot . '/mod/ddtaquiz/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquiz_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the ddtaquiz settings object. */
    protected $ddtaquiz;
     ...
     * @param object the ddtaquiz settings object.
     ...
     */
    public function __construct($submiturl, $cm, $ddtaquiz, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'ddtaquiz'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'ddtaquiz'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/ddtaquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'ddtaquiz', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'ddtaquiz'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/ddtaquiz:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/ddtaquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'ddtaquiz', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'ddtaquiz'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'ddtaquiz');
        $mform->setDefault('password', $this->ddtaquiz->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('ddtaquizopen', 'ddtaquiz'), mod_ddtaquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->ddtaquiz->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('ddtaquizclose', 'ddtaquiz'), mod_ddtaquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->ddtaquiz->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'ddtaquiz'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'ddtaquiz');
        $mform->setDefault('timelimit', $this->ddtaquiz->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'ddtaquiz'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'ddtaquiz');
        $mform->setDefault('attempts', $this->ddtaquiz->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'ddtaquiz'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'ddtaquiz'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'ddtaquiz'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one ddtaquiz setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $ddtaquiz->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'ddtaquiz');
        }
