 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_events_testcase extends advanced_testcase {
     ...
     */
    protected function prepare_ddtaquiz_data($ispreview = false) {
        ...
        // Make a user to do the ddtaquiz.
        ...
        $timenow = time();
        $attempt = ddtaquiz_create_attempt($ddtaquizobj, 1, false, $timenow, $ispreview);
        ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, 1, $timenow);
        ddtaquiz_attempt_save_started($ddtaquizobj, $quba, $attempt);

        return array($ddtaquizobj, $quba, $attempt);
    }

    public function test_attempt_submitted() {

        list($ddtaquizobj, $quba, $attempt) = $this->prepare_ddtaquiz_data();
        ...
        $event = $events[2];
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_submitted', $event);
        $this->assertEquals('ddtaquiz_attempts', $event->objecttable);
        $this->assertEquals($ddtaquizobj->get_context(), $event->get_context());
        ...
        $this->assertEquals('ddtaquiz_attempt_submitted', $event->get_legacy_eventname());
        $legacydata = new stdClass();
        $legacydata->component = 'mod_ddtaquiz';
        $legacydata->attemptid = (string) $attempt->id;
        $legacydata->timestamp = $timefinish;
        $legacydata->userid = $attempt->userid;
        $legacydata->cmid = $ddtaquizobj->get_cmid();
        $legacydata->courseid = $ddtaquizobj->get_courseid();
        $legacydata->ddtaquizid = $ddtaquizobj->get_ddtaquizid();
    ...
    public function test_attempt_becameoverdue() {
        ...
        $event = $events[0];
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_becameoverdue', $event);
        $this->assertEquals('ddtaquiz_attempts', $event->objecttable);
        $this->assertEquals($ddtaquizobj->get_context(), $event->get_context());
        $this->assertEquals($attempt->userid, $event->relateduserid);
        ...
        $this->assertEquals('ddtaquiz_attempt_overdue', $event->get_legacy_eventname());
        $legacydata = new stdClass();
        $legacydata->component = 'mod_ddtaquiz';
        $legacydata->attemptid = (string) $attempt->id;
        $legacydata->timestamp = $timefinish;
    ...
    public function test_attempt_abandoned() {
        ...
        $this->assertCount(1, $events);
        $event = $events[0];
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_abandoned', $event);
        $this->assertEquals('ddtaquiz_attempts', $event->objecttable);
        $this->assertEquals($ddtaquizobj->get_context(), $event->get_context());
        $this->assertEquals($attempt->userid, $event->relateduserid);
        // Submitterid should be the user, but as we are in PHP Unit, CLI_SCRIPT is set to true which sets null in submitterid.
        $this->assertEquals(null, $event->other['submitterid']);
        $this->assertEquals('ddtaquiz_attempt_abandoned', $event->get_legacy_eventname());
        $legacydata = new stdClass();
        $legacydata->component = 'mod_ddtaquiz';
        $legacydata->attemptid = (string) $attempt->id;
        $legacydata->timestamp = $timefinish;
    ...
    public function test_attempt_started() {
        ...
        $attempt = ddtaquiz_create_attempt($ddtaquizobj, 1, false, time(), false, 2);
        ...
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_started', $event);
        $this->assertEquals('ddtaquiz_attempts', $event->objecttable);
        $this->assertEquals($attempt->id, $event->objectid);
        $this->assertEquals($attempt->userid, $event->relateduserid);
        $this->assertEquals($ddtaquizobj->get_context(), $event->get_context());
        $this->assertEquals('ddtaquiz_attempt_started', $event->get_legacy_eventname());
        $this->assertEquals(context_module::instance($ddtaquizobj->get_cmid()), $event->get_context());
        // Check legacy log data.
        $expected = array($ddtaquizobj->get_courseid(), 'ddtaquiz', 'attempt', 'review.php?attempt=' . $attempt->id,
            $ddtaquizobj->get_ddtaquizid(), $ddtaquizobj->get_cmid());
     ...
     *
     * There is no external API for updating a ddtaquiz, so the unit test will simply
    ...
    public function test_edit_page_viewed() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\edit_page_viewed', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'editquestions', 'view.php?id=' . $ddtaquiz->cmid, $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_deleted() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        ddtaquiz_delete_attempt($attempt, $ddtaquizobj->get_ddtaquiz());
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_deleted', $event);
        $this->assertEquals(context_module::instance($ddtaquizobj->get_cmid()), $event->get_context());
        $expected = array($ddtaquizobj->get_courseid(), 'ddtaquiz', 'delete attempt', 'report.php?id=' . $ddtaquizobj->get_cmid(),
            $attempt->id, $ddtaquizobj->get_cmid());
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_preview_attempt_deleted() {
        // Create ddtaquiz with preview attempt.
        list($ddtaquizobj, $quba, $previewattempt) = $this->prepare_ddtaquiz_data(true);

        // Delete a preview attempt, capturing events.
        $sink = $this->redirectEvents();
        ddtaquiz_delete_attempt($previewattempt, $ddtaquizobj->get_ddtaquiz());
    ...
    public function test_report_viewed() {
        ...
        $params = array(
            'context' => $context = context_module::instance($ddtaquiz->cmid),
            'other' => array(
                'ddtaquizid' => $ddtaquiz->id,
                'reportname' => 'overview'
            )
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\report_viewed', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'report', 'report.php?id=' . $ddtaquiz->cmid . '&mode=overview',
            $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_reviewed() {
        ...
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'review', 'review.php?attempt=1', $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_summary_viewed() {
        ...
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'view summary', 'summary.php?attempt=1', $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_created() {
        ...
        $event = \mod_ddtaquiz\event\user_override_created::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\user_override_created', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_created() {
        ...
        $event = \mod_ddtaquiz\event\group_override_created::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\group_override_created', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_updated() {
        ...
        $event = \mod_ddtaquiz\event\user_override_updated::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\user_override_updated', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'edit override', 'overrideedit.php?id=1', $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_updated() {
        ...
        $event = \mod_ddtaquiz\event\group_override_updated::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\group_override_updated', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'edit override', 'overrideedit.php?id=1', $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_override_deleted() {
        ...
        $override->ddtaquiz = $ddtaquiz->id;
        $override->userid = 2;
        $override->id = $DB->insert_record('ddtaquiz_overrides', $override);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\user_override_deleted', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'delete override', 'overrides.php?cmid=' . $ddtaquiz->cmid, $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_deleted() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\group_override_deleted', $event);
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'delete override', 'overrides.php?cmid=' . $ddtaquiz->cmid, $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_attempt_viewed() {
        ...
        $this->assertEquals(context_module::instance($ddtaquiz->cmid), $event->get_context());
        $expected = array($course->id, 'ddtaquiz', 'continue attempt', 'review.php?attempt=1', $ddtaquiz->id, $ddtaquiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_preview_started() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\attempt_preview_started', $event);
        $this->assertEquals(context_module::instance($ddtaquizobj->get_cmid()), $event->get_context());
        $expected = array($ddtaquizobj->get_courseid(), 'ddtaquiz', 'preview', 'view.php?id=' . $ddtaquizobj->get_cmid(),
            $ddtaquizobj->get_ddtaquizid(), $ddtaquizobj->get_cmid());
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_question_manually_graded() {
        ...
        $params = array(
            ...
            'context' => context_module::instance($ddtaquizobj->get_cmid()),
            'other' => array(
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_ddtaquiz\event\question_manually_graded', $event);
        $this->assertEquals(context_module::instance($ddtaquizobj->get_cmid()), $event->get_context());
        $expected = array($ddtaquizobj->get_courseid(), 'ddtaquiz', 'manualgrade', 'comment.php?attempt=2&slot=3',
            $ddtaquizobj->get_ddtaquizid(), $ddtaquizobj->get_cmid());
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
