...
/**
 * Ddtaquiz attempt walk through using data from csv file.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the ddtaquiz record we create.
     */
    protected $ddtaquiz;
     ...
     * directory.
     *
     * @param array $ddtaquizsettings of settings read from csv file ddtaquizzes.csv
     ...
     */
    public function test_walkthrough_from_csv($ddtaquizsettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-ddtaquiz-tools/tree/master/responsegenerator

        $this->create_ddtaquiz_simulate_attempts_and_check_results($ddtaquizsettings, $csvdata);
    }

    public function create_ddtaquiz($ddtaquizsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $ddtaquizsettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->ddtaquiz = $ddtaquizgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                ddtaquiz_add_ddtaquiz_question($slotquestion['id'], $this->ddtaquiz, 0, $slotquestion['mark']);
            } else {
                ddtaquiz_add_random_questions($this->ddtaquiz, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create ddtaquiz, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $ddtaquizsettings Ddtaquiz overrides for this ddtaquiz.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_ddtaquiz_simulate_attempts_and_check_results($ddtaquizsettings, $csvdata) {
        $this->resetAfterTest(true);
        question_bank::get_qtype('random')->clear_caches_before_testing();

        $this->create_ddtaquiz($ddtaquizsettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough() {
        $ddtaquizzes = $this->load_csv_data_file('ddtaquizzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $ddtaquizzes->getRowCount(); $rowno++) {
            $ddtaquizsettings = $ddtaquizzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $ddtaquizsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $ddtaquizsettings['testnumber']);
                }
            }
            $datasets[] = array($ddtaquizsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the ddtaquiz_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the ddtaquiz.
            ...
            if (!isset($attemptids[$step['ddtaquizattempt']])) {
                // Start the attempt.
                $ddtaquizobj = ddtaquiz::create($this->ddtaquiz->id, $user->id);
                ...
                $prevattempts = ddtaquiz_get_user_attempts($this->ddtaquiz->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = ddtaquiz_create_attempt($ddtaquizobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                ddtaquiz_attempt_save_started($ddtaquizobj, $quba, $attempt);
                $attemptid = $attemptids[$step['ddtaquizattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['ddtaquizattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the ddtaquiz_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load ddtaquiz attempt data.
            $attemptobj = ddtaquiz_attempt::create($attemptids[$result['ddtaquizattempt']]);
     ...
     * @param ddtaquiz_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['ddtaquizattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['ddtaquizattempt']}.");
                    break;
                case 'ddtaquizgrade' :
                    // Check ddtaquiz grades.
                    $grades = ddtaquiz_get_user_grades($attemptobj->get_ddtaquiz(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Ddtaquiz grade for attempt {$result['ddtaquizattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'ddtaquiz',
                                                        $attemptobj->get_ddtaquizid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['ddtaquizattempt']}.");
                    break;
                default :
