...
/**
 * Tests for the ddtaquiz_attempt class.
 *
...
/**
 * Subclass of ddtaquiz_attempt to allow faking of the page layout.
 *
 * @copyright 2014 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempt_testable extends ddtaquiz_attempt {
     ...
     * @param string $layout layout to set. Like ddtaquiz attempt.layout. E.g. '1,2,0,3,4,0,'.
     * @param array $infos slot numbers which contain 'descriptions', or other non-questions.
     * @return ddtaquiz_attempt attempt object for use in unit tests.
     */
    public static function setup_fake_attempt_layout($id, $layout, $infos = array()) {
        ...
        $attemptobj = new self($attempt, $ddtaquiz, $cm, $course, false);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempt_testcase extends basic_testcase {
    /**
     * Test the functions ddtaquiz_update_open_attempts() and get_list_of_overdue_attempts()
     */
    public function test_attempt_url() {
        $attempt = mod_ddtaquiz_attempt_testable::setup_fake_attempt_layout(
                123, '1,2,0,3,4,0,5,6,0');

        // Attempt pages.
        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/attempt.php?attempt=123&cmid=0'),
                $attempt->attempt_url());

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/attempt.php?attempt=123&page=2&cmid=0'),
                $attempt->attempt_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/attempt.php?attempt=123&page=1&cmid=0#'),
                $attempt->attempt_url(3));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/attempt.php?attempt=123&page=1&cmid=0#q4'),
        ...
        // Summary page.
        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/summary.php?attempt=123&cmid=0'),
                $attempt->summary_url());

        // Review page.
        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=123&cmid=0'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=123&page=2&cmid=0'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=123&page=1&cmid=0'),
                $attempt->review_url(3, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=123&page=1&cmid=0#q4'),
        ...
        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=123&showall=0&cmid=0'),
        ...
        // Review with more than 50 questions in the ddtaquiz.
        $attempt = mod_ddtaquiz_attempt_testable::setup_fake_attempt_layout(
        ...
        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=124&cmid=0'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=124&page=2&cmid=0'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=124&page=1&cmid=0'),
                $attempt->review_url(11, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=124&page=1&cmid=0#q12'),
                $attempt->review_url(12, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/ddtaquiz/review.php?attempt=124&showall=1&cmid=0'),
                $attempt->review_url(null, 2, true));
