...
/**
 * Ddtaquiz attempt walk through tests.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a ddtaquiz with questions and walk through a ddtaquiz attempt.
     */
    public function test_ddtaquiz_attempt_walkthrough() {
        global $SITE;
        ...
        $ddtaquiz = $ddtaquizgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $grades = ddtaquiz_get_user_grades($ddtaquiz, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'ddtaquiz', $ddtaquiz->id, $user1->id);
    ...
    /**
     * Create a ddtaquiz with a random as well as other questions and walk through ddtaquiz attempts.
     */
    public function test_ddtaquiz_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $ddtaquiz = $ddtaquizgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the ddtaquiz.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = ddtaquiz_create_attempt($ddtaquizobj, 1, false, $timenow);

            ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $ddtaquizwithvariants = null;

    /**
     * Create a ddtaquiz with a single question with variants and walk through ddtaquiz attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_ddtaquiz_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->ddtaquizwithvariants === null) {
            // Make a ddtaquiz.
            $ddtaquizgenerator = $this->getDataGenerator()->get_plugin_generator('mod_ddtaquiz');

            $this->ddtaquizwithvariants = $ddtaquizgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            ddtaquiz_add_ddtaquiz_question($calc->id, $this->ddtaquizwithvariants, 0);
        }


        // Make a new user to do the ddtaquiz.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $ddtaquizobj = ddtaquiz::create($this->ddtaquizwithvariants->id, $user1->id);
        ...
        // Select variant.
        ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check ddtaquiz grades.
        $grades = ddtaquiz_get_user_grades($this->ddtaquizwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'ddtaquiz', $this->ddtaquizwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
