...
/**
 * Ddtaquiz attempt overdue handling tests
 *
...
/**
 * Unit tests for ddtaquiz attempt overdue handling
 *
 * @package    mod_ddtaquiz
 * @category   phpunit
 * @copyright  2012 Matt Petro
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_attempt_overdue_testcase extends advanced_testcase {
    /**
     * Test the functions ddtaquiz_update_open_attempts() and get_list_of_overdue_attempts()
     */
    public function test_bulk_update_functions() {
        ...
        $ddtaquiz_generator = $this->getDataGenerator()->get_plugin_generator('mod_ddtaquiz');

        // Basic ddtaquiz settings

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1200, 'timelimit'=>600, 'message'=>'Test1A');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>1800));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1200, 'timelimit'=>1800, 'message'=>'Test1B');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1200, 'timelimit'=>0, 'message'=>'Test1C');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>600));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>0, 'timelimit'=>600, 'message'=>'Test1D');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>null));
        $usertimes[$attemptid] = array('timeclose'=>1300, 'timelimit'=>0, 'message'=>'Test2A');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>0));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1100, 'timelimit'=>null));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1100, 'timelimit'=>0, 'message'=>'Test2B');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>null, 'timelimit'=>700));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>0, 'timelimit'=>700, 'message'=>'Test2C');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>null, 'timelimit'=>500));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>0, 'timelimit'=>500, 'message'=>'Test2D');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>null, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>0, 'timelimit'=>0, '', 'message'=>'Test2E');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>500));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1300, 'timelimit'=>500, '', 'message'=>'Test2F');
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>1000, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++, 'attempt'=>1));
        $usertimes[$attemptid] = array('timeclose'=>1300, 'timelimit'=>500, '', 'message'=>'Test2G');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group3->id, 'timeclose'=>1300, 'timelimit'=>500)); // user not in group
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>501));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1301, 'timelimit'=>500));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1301, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1300, 'timelimit'=>501));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1301, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1300, 'timelimit'=>501));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group3->id, 'timeclose'=>1500, 'timelimit'=>1000)); // user not in group
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>null, 'timelimit'=>501));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1301, 'timelimit'=>null));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1301, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>500));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>0, 'timelimit'=>501));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'timeclose'=>1201, 'timelimit'=>601));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'timeclose'=>0, 'timelimit'=>601));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'timeclose'=>1201, 'timelimit'=>0));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'timeclose'=>null, 'timelimit'=>601));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'timeclose'=>1201, 'timelimit'=>null));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group1->id, 'timeclose'=>1300, 'timelimit'=>700));
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>0, 'timeclose'=>1201, 'timelimit'=>601)); // not user
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'inprogress', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        ...
        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>1200, 'timelimit'=>600, 'overduehandling'=>'graceperiod', 'graceperiod'=>250));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'overdue', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>1200, 'timelimit'=>600, '', 'message'=>'Test5A');

        $ddtaquiz = $ddtaquiz_generator->create_instance(array('course'=>$course->id, 'timeclose'=>0, 'timelimit'=>600, 'overduehandling'=>'graceperiod', 'graceperiod'=>250));
        $attemptid = $DB->insert_record('ddtaquiz_attempts', array('ddtaquiz'=>$ddtaquiz->id, 'userid'=>$user1->id, 'state'=>'overdue', 'timestart'=>100, 'timecheckstate'=>0, 'layout'=>'', 'uniqueid'=>$uniqueid++));
        $usertimes[$attemptid] = array('timeclose'=>0, 'timelimit'=>600, '', 'message'=>'Test5B');

        //
        // Test ddtaquiz_update_open_attempts()
        //

        ddtaquiz_update_open_attempts(array('courseid'=>$course->id));
        foreach ($usertimes as $attemptid=>$times) {
            $attempt = $DB->get_record('ddtaquiz_attempts', array('id'=>$attemptid));
            $this->assertTrue(false !== $attempt, $times['message']);

            if ($attempt->state == 'overdue') {
                $graceperiod = $DB->get_field('ddtaquiz', 'graceperiod', array('id'=>$attempt->ddtaquiz));
            } else {
                $graceperiod = 0;
            }
        ...
        $attempts->close();
        $this->assertEquals($DB->count_records_select('ddtaquiz_attempts', 'timecheckstate IS NOT NULL'), $count);
    ...
    public function test_group_event_handlers() {
        ...
        $this->assertEquals(1300, $DB->get_field('ddtaquiz_attempts', 'timecheckstate', array('id'=>$attemptid)));

        // remove from group
        $this->assertTrue(groups_remove_member($group1, $user1));
        $this->assertEquals(1200, $DB->get_field('ddtaquiz_attempts', 'timecheckstate', array('id'=>$attemptid)));
        ...
        $this->assertEquals(1200, $DB->get_field('ddtaquiz_attempts', 'timecheckstate', array('id'=>$attemptid)));
        $this->assertEquals(0, $DB->count_records('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id)));

        // add a group2 override
        $DB->insert_record('ddtaquiz_overrides', array('ddtaquiz'=>$ddtaquiz->id, 'groupid'=>$group2->id, 'timeclose'=>1400, 'timelimit'=>null));
        ddtaquiz_update_open_attempts(array('ddtaquizid'=>$ddtaquiz->id));
        $this->assertEquals(1400, $DB->get_field('ddtaquiz_attempts', 'timecheckstate', array('id'=>$attemptid)));
    ...
    /**
     * Test the functions ddtaquiz_create_attempt_handling_errors
     */
    public function test_ddtaquiz_create_attempt_handling_errors() {
        ...
        $ddtaquiz = $ddtaquizgenerator->create_instance(array('course' => $course->id, 'questionsperpage' => 0, 'grade' => 100.0,
            'sumgrades' => 2));
        // Create questions.
        $cat = $questiongenerator->create_question_category();
        $saq = $questiongenerator->create_question('shortanswer', null, array('category' => $cat->id));
        $numq = $questiongenerator->create_question('numerical', null, array('category' => $cat->id));
        // Add them to the ddtaquiz.
        ddtaquiz_add_ddtaquiz_question($saq->id, $ddtaquiz);
        ddtaquiz_add_ddtaquiz_question($numq->id, $ddtaquiz);
        $ddtaquizobj = ddtaquiz::create($ddtaquiz->id, $user1->id);
        ...
        $timenow = time();
        // Create an attempt.
        $attempt = ddtaquiz_create_attempt($ddtaquizobj, 1, false, $timenow, false, $user1->id);
        ddtaquiz_start_new_attempt($ddtaquizobj, $quba, $attempt, 1, $timenow);
        ddtaquiz_attempt_save_started($ddtaquizobj, $quba, $attempt);
        $result = ddtaquiz_create_attempt_handling_errors($attempt->id, $ddtaquiz->cmid);
        $this->assertEquals($result->get_attemptid(), $attempt->id);
        try {
            $result = ddtaquiz_create_attempt_handling_errors($attempt->id, 9999);
            $this->fail('Exception expected due to invalid course module id.');
        } catch (moodle_exception $e) {
            $this->assertEquals('invalidcoursemodule', $e->errorcode);
        }
        try {
            ddtaquiz_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to ddtaquiz content change.');
        } catch (moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchange', $e->errorcode);
        }
        try {
            ddtaquiz_create_attempt_handling_errors(9999);
            $this->fail('Exception expected due to invalid ddtaquiz attempt id.');
        } catch (moodle_exception $e) {
        ...
        try {
            ddtaquiz_create_attempt_handling_errors(9999, $result->get_cmid());
            $this->fail('Exception expected due to ddtaquiz content change for user without permission.');
        } catch (moodle_exception $e) {
            $this->assertEquals('attempterrorcontentchangeforuser', $e->errorcode);
        }
        try {
            ddtaquiz_create_attempt_handling_errors($attempt->id, 9999);
