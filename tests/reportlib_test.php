...
/**
 * Unit tests for (some of) mod/ddtaquiz/report/reportlib.php
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_ddtaquiz_reportlib_testcase extends advanced_testcase {
    public function test_ddtaquiz_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = ddtaquiz_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = ddtaquiz_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_ddtaquiz_report_scale_summarks_as_percentage() {
        $ddtaquiz = new stdClass();
        $ddtaquiz->sumgrades = 10;
        $ddtaquiz->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            ddtaquiz_report_scale_summarks_as_percentage(1.234567, $ddtaquiz, false));
        $this->assertEquals('12.35%',
            ddtaquiz_report_scale_summarks_as_percentage(1.234567, $ddtaquiz, true));
        $this->assertEquals('-',
            ddtaquiz_report_scale_summarks_as_percentage('-', $ddtaquiz, true));
    }

    public function test_ddtaquiz_report_qm_filter_select_only_one_attempt_allowed() {
        $ddtaquiz = new stdClass();
        $ddtaquiz->attempts = 1;
        $this->assertSame('', ddtaquiz_report_qm_filter_select($ddtaquiz));
    }

    public function test_ddtaquiz_report_qm_filter_select_average() {
        $ddtaquiz = new stdClass();
        $ddtaquiz->attempts = 10;
        $ddtaquiz->grademethod = DDTAQUIZ_GRADEAVERAGE;
        $this->assertSame('', ddtaquiz_report_qm_filter_select($ddtaquiz));
    }

    public function test_ddtaquiz_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->ddtaquiz = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = ddtaquiz_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | ddtaquizid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('ddtaquiz_attempts', $fakeattempt);
        ...
        $fakeattempt->state = ddtaquiz_attempt::IN_PROGRESS;
        $DB->insert_record('ddtaquiz_attempts', $fakeattempt);
        ...
        $ddtaquiz->grademethod = DDTAQUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {ddtaquiz_attempts} ddtaquiza WHERE userid = ? AND ddtaquiz = ? AND "
                        . ddtaquiz_report_qm_filter_select($ddtaquiz), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $ddtaquiz->grademethod = DDTAQUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {ddtaquiz_attempts} qa_alias WHERE userid = ? AND ddtaquiz = ? AND "
                . ddtaquiz_report_qm_filter_select($ddtaquiz, 'qa_alias'), array(123, 456));
