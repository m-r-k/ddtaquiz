...
/**
 * Unit tests for the {@link \mod_ddtaquiz\repaginate} class.
...
require_once($CFG->dirroot . '/mod/ddtaquiz/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_ddtaquiz\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_repaginate_testable extends \mod_ddtaquiz\repaginate {

    public function __construct($ddtaquizid = 0, $slots = null) {
        return parent::__construct($ddtaquizid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $ddtaquizslots;
    /** @var mod_ddtaquiz_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_ddtaquiz_slots($this->get_ddtaquiz_object()->get_slots());
        $this->repaginate = new mod_ddtaquiz_repaginate_testable(0, $this->ddtaquizslots);
    }
    ...
    /**
     * Create a ddtaquiz, add five questions to the ddtaquiz
     * which are all on one page and return the ddtaquiz object.
     */
    private function get_ddtaquiz_object() {
        global $SITE;
        ...
        $ddtaquiz = $ddtaquizgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('ddtaquiz', $ddtaquiz->id, $SITE->id);
        ...
        ddtaquiz_add_ddtaquiz_question($shortanswer->id, $ddtaquiz);
        ddtaquiz_add_ddtaquiz_question($numerical->id, $ddtaquiz);
        ddtaquiz_add_ddtaquiz_question($essay->id, $ddtaquiz);
        ddtaquiz_add_ddtaquiz_question($truefalse->id, $ddtaquiz);
        ddtaquiz_add_ddtaquiz_question($match->id, $ddtaquiz);

        // Return the ddtaquiz object.
        $ddtaquizobj = new ddtaquiz($ddtaquiz, $cm, $SITE);
        return \mod_ddtaquiz\structure::create_for_ddtaquiz($ddtaquizobj);
    }

    /**
     * Set the ddtaquiz slots
     * @param string $slots
     */
    private function set_ddtaquiz_slots($slots = null) {
        if (!$slots) {
            $this->ddtaquizslots = $this->get_ddtaquiz_object()->get_slots();
        } else {
            $this->ddtaquizslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_ddtaquiz_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->ddtaquizslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->ddtaquizslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->ddtaquizslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->ddtaquizslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->ddtaquizslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->ddtaquizslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->ddtaquizslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->ddtaquizslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->ddtaquizslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->ddtaquizslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_ddtaquiz_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->ddtaquizslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_ddtaquiz_slots();
        $slotfrom = 1;
        $type = \mod_ddtaquiz\repaginate::LINK;
        $expected = array();
        foreach ($this->ddtaquizslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->ddtaquizslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->ddtaquizslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_ddtaquiz\repaginate::UNLINK;
        $expected = array();
        foreach ($this->ddtaquizslots as $slot) {
