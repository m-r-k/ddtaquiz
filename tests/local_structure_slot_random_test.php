...
/**
 * Unit tests for the {@link \mod_ddtaquiz\local\structure\slot_random} class.
 *
...
/**
 * Class mod_ddtaquiz_local_structure_slot_random_test
 * Class for tests related to the {@link \mod_ddtaquiz\local\structure\slot_random} class.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_ddtaquiz_local_structure_slot_random_test extends advanced_testcase {
    /**
     * Constructor test.
     */
    public function test_constructor() {
        ...
        // Create a random question without adding it to a ddtaquiz.
        // We don't want to use ddtaquiz_add_random_questions because that itself, instantiates an object from the slot_random class.
        $form = new stdClass();
        ...
        $rc = new ReflectionClass('\mod_ddtaquiz\local\structure\slot_random');
        $rcp = $rc->getProperty('record');
        $rcp->setAccessible(true);
        $record = $rcp->getValue($randomslot);

        $this->assertEquals($ddtaquiz->id, $record->ddtaquizid);
    ...
    public function test_get_ddtaquiz_ddtaquiz() {
        global $SITE, $DB;
        ...
        ddtaquiz_add_random_questions($ddtaquiz, 0, $category->id, 1, false);

        // Get the random question's id. It is at the first slot.
        $questionid = $DB->get_field('ddtaquiz_slots', 'questionid', array('ddtaquizid' => $ddtaquiz->id, 'slot' => 1));
        ...
        // The create_instance had injected an additional cmid propery to the ddtaquiz. Let's remove that.
        unset($ddtaquiz->cmid);

        $this->assertEquals($ddtaquiz, $randomslot->get_ddtaquiz());
    }

    public function test_set_ddtaquiz() {
        global $SITE, $DB;
        ...
        $rc = new ReflectionClass('\mod_ddtaquiz\local\structure\slot_random');
        $rcp = $rc->getProperty('ddtaquiz');
        $rcp->setAccessible(true);
        $ddtaquizpropery = $rcp->getValue($randomslot);

        $this->assertEquals($ddtaquiz, $ddtaquizpropery);
    }
    ...
    public function test_insert() {
        ...
        // Insert the random question to the ddtaquiz.
        $randomslot = new \mod_ddtaquiz\local\structure\slot_random($randomslotdata);
        $randomslot->set_tags([$footag, $bartag]);
        $randomslot->insert(1); // Put the question on the first page of the ddtaquiz.

        // Get the random question's ddtaquiz_slot. It is at the first slot.
        $ddtaquizslot = $DB->get_record('ddtaquiz_slots', array('ddtaquizid' => $ddtaquiz->id, 'slot' => 1));
        // Get the random question's tags from ddtaquiz_slot_tags. It is at the first slot.
        $ddtaquizslottags = $DB->get_records('ddtaquiz_slot_tags', array('slotid' => $ddtaquizslot->id));

        $this->assertEquals($question->id, $ddtaquizslot->questionid);
        $this->assertEquals($category->id, $ddtaquizslot->questioncategoryid);
        $this->assertEquals(1, $ddtaquizslot->includingsubcategories);
        $this->assertEquals(1, $ddtaquizslot->maxmark);

        $this->assertCount(2, $ddtaquizslottags);
        $this->assertEquals(
                [
