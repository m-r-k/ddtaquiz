...
$reportlist = ddtaquiz_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see ddtaquiz reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/ddtaquiz/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = ddtaquiz_get_user_attempts($ddtaquiz->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new ddtaquiz_attempt($attempt, $ddtaquiz, $cm, $course, false);
...
redirect(new moodle_url('/mod/ddtaquiz/report.php', array(
