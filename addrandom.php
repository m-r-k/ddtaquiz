...
/**
 * Fallback page of /mod/ddtaquiz/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $ddtaquiz, $pagevars) =
        question_edit_setup('editq', '/mod/ddtaquiz/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/ddtaquiz/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new ddtaquiz_add_random_form(new moodle_url('/mod/ddtaquiz/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    ddtaquiz_add_random_questions($ddtaquiz, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingddtaquiz = get_string('editinga', 'moodle', get_string('modulename', 'ddtaquiz'));
$PAGE->navbar->add($streditingddtaquiz);
$PAGE->set_title($streditingddtaquiz);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$ddtaquizname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoddtaquiz', 'ddtaquiz', $ddtaquizname), 2);
$mform->display();
echo $OUTPUT->footer();
