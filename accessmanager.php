...
/**
 * Classes to enforce the various access rules that can apply to a ddtaquiz.
 *
 ...
 * ddtaquiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class ddtaquiz_access_manager {
    /** @var ddtaquiz the ddtaquiz settings object. */
    protected $ddtaquizobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of ddtaquiz_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular ddtaquiz.
     * @param object $ddtaquizobj An instance of the class ddtaquiz from attemptlib.php.
     *      The ddtaquiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/ddtaquiz:ignoretimelimits', ...)).
     */
    public function __construct($ddtaquizobj, $timenow, $canignoretimelimits) {
        $this->ddtaquizobj = $ddtaquizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($ddtaquizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular ddtaquiz.
     * @param ddtaquiz $ddtaquizobj information about the ddtaquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/ddtaquiz:ignoretimelimits capability.
     * @return array of {@link ddtaquiz_access_rule_base}s.
     */
    protected function make_rules($ddtaquizobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($ddtaquizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['ddtaquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('ddtaquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the ddtaquiz table.
     *
     * @param mod_ddtaquiz_mod_form $ddtaquizform the ddtaquiz settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($ddtaquizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'ddtaquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_ddtaquiz_mod_form $ddtaquizform the ddtaquiz form object.
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $ddtaquizform);
        }
    ...
    /**
     * Save any submitted settings when the ddtaquiz settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the ddtaquiz table.
     *
     * @param object $ddtaquiz the data from the ddtaquiz form, including $ddtaquiz->id
     *      which is the id of the ddtaquiz being saved.
     */
    public static function save_settings($ddtaquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($ddtaquiz);
        }
    }

    /**
     * Delete any rule-specific settings when the ddtaquiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the ddtaquiz table.
     *
     * @param object $ddtaquiz the data from the database, including $ddtaquiz->id
     *      which is the id of the ddtaquiz being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($ddtaquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($ddtaquiz);
        }
    }
     ...
     */
    protected static function get_load_sql($ddtaquizid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{ddtaquiz} ddtaquiz';
        $allparams = array('ddtaquizid' => $ddtaquizid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($ddtaquizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE ddtaquiz.id = :ddtaquizid", $allparams);
    }
     ...
     */
    public static function load_settings($ddtaquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($ddtaquizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($ddtaquizid);
        }
    ...
    /**
     * Load the ddtaquiz settings and any settings required by the access rules.
     ...
     *
     * @param int $ddtaquizid the ddtaquiz id.
     * @return object mdl_ddtaquiz row with extra fields.
     */
    public static function load_ddtaquiz_and_settings($ddtaquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($ddtaquizid, $rules, 'ddtaquiz.*');
        $ddtaquiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($ddtaquizid) as $name => $value) {
                $ddtaquiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this ddtaquiz, such
     * as is shown at the top of the ddtaquiz view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this ddtaquiz now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_ddtaquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_ddtaquiz_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_ddtaquiz_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'ddtaquizobj' => $this->ddtaquizobj,
     ...
     * ddtaquiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant ddtaquiz_attempts row.
    ...
    /**
     * @return bolean if this ddtaquiz should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the ddtaquiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_ddtaquiz_renderer $output the ddtaquiz renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->ddtaquizobj->view_url(), $message);
            die();
        } else {
            redirect($this->ddtaquizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the ddtaquiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(ddtaquiz_attempt::IN_PROGRESS, ddtaquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = ddtaquiz_attempt_state($this->ddtaquizobj->get_ddtaquiz(), $attempt);
        $reviewoptions = mod_ddtaquiz_display_options::make_from_ddtaquiz(
                $this->ddtaquizobj->get_ddtaquiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->ddtaquizobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->ddtaquizobj->review_url($attempt->id),
