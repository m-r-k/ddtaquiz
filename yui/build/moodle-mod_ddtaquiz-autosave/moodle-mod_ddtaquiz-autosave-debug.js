YUI.add('moodle-mod_ddtaquiz-autosave', function (Y, NAME) {
...
/**
 * Auto-save functionality for during ddtaquiz attempts.
 *
 * @module moodle-mod_ddtaquiz-autosave
 */

/**
 * Auto-save functionality for during ddtaquiz attempts.
 *
 * @class M.mod_ddtaquiz.autosave
 */

M.mod_ddtaquiz = M.mod_ddtaquiz || {};
M.mod_ddtaquiz.autosave = {
    /**
     ...
     * @type String
     * @default M.cfg.wwwroot + '/mod/ddtaquiz/autosave.ajax.php'
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/ddtaquiz/autosave.ajax.php',
     ...
     */
    init: function(delay) {
        this.form = Y.one(this.SELECTORS.DDTAQUIZ_FORM);
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-mod_ddtaquiz-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof window.tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-mod_ddtaquiz-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-mod_ddtaquiz-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-mod_ddtaquiz-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-mod_ddtaquiz-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-mod_ddtaquiz-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-mod_ddtaquiz-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-mod_ddtaquiz-autosave');
        if (typeof window.tinyMCE !== 'undefined') {
    ...
    save_done: function(transactionid, response) {
        ...
        Y.log('Save completed.', 'debug', 'moodle-mod_ddtaquiz-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-mod_ddtaquiz-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-mod_ddtaquiz-autosave');
        this.save_transaction = null;
    ...
    is_time_nearly_over: function() {
        return M.mod_ddtaquiz.timer && M.mod_ddtaquiz.timer.endtime &&
                (new Date().getTime() + 2 * this.delay) > M.mod_ddtaquiz.timer.endtime;
    },
