YUI.add('moodle-mod_ddtaquiz-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW: 'a.' + CSS.SHOW,
        SLOTLI: 'li.slot',
        SUMMARKS: '.mod_ddtaquiz_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.ddtaquizid = this.get('ddtaquizid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'ddtaquiz', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a ddtaquiz in editing mode.
 *
 * @module mod_ddtaquiz-resource-toolbox
 * @namespace M.mod_ddtaquiz.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_ddtaquiz.ddtaquizbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'ddtaquiz', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_ddtaquiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_ddtaquiz.util.slot.remove(element);
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_ddtaquiz.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_ddtaquiz.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-ddtaquiz-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'ddtaquiz'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_ddtaquiz.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_ddtaquiz.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_ddtaquiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_ddtaquiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_ddtaquiz.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_ddtaquiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_ddtaquiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_ddtaquiz.util.slot.reorderSlots();
        Y.Moodle.mod_ddtaquiz.util.slot.reorderPageBreaks();
        Y.Moodle.mod_ddtaquiz.util.page.reorderPages();
        Y.Moodle.mod_ddtaquiz.util.slot.updateOneSlotSections();
        Y.Moodle.mod_ddtaquiz.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_ddtaquiz-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_ddtaquiz.resource_toolbox = null;
M.mod_ddtaquiz.init_resource_toolbox = function(config) {
    M.mod_ddtaquiz.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_ddtaquiz.resource_toolbox;
};
/* global TOOLBOX, BODY, SELECTOR */
 ...
 *
 * @module moodle-mod_ddtaquiz-toolboxes
 * @namespace M.mod_ddtaquiz.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a ddtaquiz.
 *
 * @class section
 * @constructor
 * @extends M.mod_ddtaquiz.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'ddtaquiz', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'ddtaquiz', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'ddtaquiz', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'ddtaquiz', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'ddtaquiz', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_ddtaquiz-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_ddtaquiz.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_ddtaquiz-ddtaquizbase",
        "moodle-mod_ddtaquiz-util-slot",
