...
/**
 * Defines the \mod_ddtaquiz\structure class.
 *
...
namespace mod_ddtaquiz;
defined('MOODLE_INTERNAL') || die();

/**
 * Ddtaquiz structure class.
 *
 * The structure of the ddtaquiz. That is, which questions it is built up
 * from. This is used on the Edit ddtaquiz page (edit.php) and also when
 * starting an attempt at the ddtaquiz (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \ddtaquiz the ddtaquiz this is the structure of. */
    protected $ddtaquizobj = null;

    /**
     * @var \stdClass[] the questions in this ddtaquiz. Contains the row from the questions
     * table, with the data from the ddtaquiz_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] ddtaquiz_slots.id => the ddtaquiz_slots rows for this ddtaquiz, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] ddtaquiz_slots.slot => the ddtaquiz_slots rows for this ddtaquiz, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * ddtaquiz_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty ddtaquiz.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given ddtaquiz.
     * @param \ddtaquiz $ddtaquizobj the ddtaquiz.
     * @return structure
     */
    public static function create_for_ddtaquiz($ddtaquizobj) {
        $structure = self::create();
        $structure->ddtaquizobj = $ddtaquizobj;
        $structure->populate_structure($ddtaquizobj->get_ddtaquiz());
        return $structure;
    }

    /**
     * Whether there are any questions in the ddtaquiz.
     * @return bool true if there is at least one question in the ddtaquiz.
     */
    ...
    /**
     * Get the number of questions in the ddtaquiz.
     * @return int the number of questions in the ddtaquiz.
     */
     ...
     * question_category.contextid, and the ddtaquiz_slots data for the question in this ddtaquiz.
     */
     ...
     * questions cannot be depended upon because of ddtaquiz options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->ddtaquizobj->get_navigation_method() == DDTAQUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->ddtaquizobj->get_ddtaquiz()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_ddtaquiz', $this->ddtaquizobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->ddtaquizobj->get_ddtaquiz()->preferredbehaviour);
    ...
    /**
     * Get the course id that the ddtaquiz belongs to.
     * @return int the course.id for the ddtaquiz.
     */
    ...
    /**
     * Get the course module id of the ddtaquiz.
     * @return int the course_modules.id for the ddtaquiz.
     */
    ...
    /**
     * Get id of the ddtaquiz.
     * @return int the ddtaquiz.id for the ddtaquiz.
     */
     ...
     * @return \stdClass the ddtaquiz settings row from the database.
     */
    ...
    /**
     * Ddtaquizzes can only be repaginated if they have not been attempted, the
     * questions are not shuffled, and there are two or more questions.
     * @return bool whether this ddtaquiz can be repaginated.
     */
    ...
    /**
     * Ddtaquizzes can only be edited if they have not been attempted.
     * @return bool whether the ddtaquiz can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !ddtaquiz_has_attempts($this->ddtaquizobj->get_ddtaquizid());
        }
        return $this->canbeedited;
    }

    /**
     * This ddtaquiz can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = ddtaquiz_attempt_summary_link_to_reports($this->get_ddtaquiz(),
                    $this->ddtaquizobj->get_cm(), $this->ddtaquizobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'ddtaquiz',
                    new \moodle_url('/mod/ddtaquiz/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the ddtaquiz.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the ddtaquiz.
     * @return int the number of questions that should be on each page of the
     * ddtaquiz by default.
     */
    public function get_questions_per_page() {
        return $this->ddtaquizobj->get_ddtaquiz()->questionsperpage;
    }

    /**
     * Get ddtaquiz slots.
     * @return \stdClass[] the slots in this ddtaquiz.
     */
    ...
    /**
     * Is this slot the last one in the ddtaquiz?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the ddtaquiz.
     */
    public function is_last_slot_in_ddtaquiz($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the ddtaquiz?
     * @param \stdClass $section the ddtaquiz_sections row.
     * @return bool whether this is first section in the ddtaquiz.
     */
    ...
    /**
     * Is this the last section in the ddtaquiz?
     * @param \stdClass $section the ddtaquiz_sections row.
     * @return bool whether this is first section in the ddtaquiz.
     */
    ...
    /**
     * Get the final slot in the ddtaquiz.
     * @return \stdClass the ddtaquiz_slots for for the final slot in the ddtaquiz.
     */
     ...
     * @return \stdClass the requested ddtaquiz_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the ddtaquiz.
    ...
    /**
     * Get all the sections of the ddtaquiz.
     * @return \stdClass[] the sections in this ddtaquiz.
     */
    ...
    /**
     * Get the overall ddtaquiz grade formatted for display.
     * @return string the maximum grade for this ddtaquiz.
     */
    public function formatted_ddtaquiz_grade() {
        return ddtaquiz_format_grade($this->get_ddtaquiz(), $this->get_ddtaquiz()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return ddtaquiz_format_question_grade($this->get_ddtaquiz(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall ddtaquiz grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_ddtaquiz()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return ddtaquiz_get_grade_format($this->get_ddtaquiz());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (ddtaquiz_has_attempts($this->ddtaquizobj->get_ddtaquizid())) {
            $reviewlink = ddtaquiz_attempt_summary_link_to_reports($this->ddtaquizobj->get_ddtaquiz(),
                    $this->ddtaquizobj->get_cm(), $this->ddtaquizobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'ddtaquiz', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the ddtaquiz.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $ddtaquiz = $this->ddtaquizobj->get_ddtaquiz();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($ddtaquiz->timeopen > 0) {
            if ($timenow > $ddtaquiz->timeopen) {
                $dates[] = get_string('ddtaquizopenedon', 'ddtaquiz', userdate($ddtaquiz->timeopen));
            } else {
                $dates[] = get_string('ddtaquizwillopen', 'ddtaquiz', userdate($ddtaquiz->timeopen));
            }
        }
        if ($ddtaquiz->timeclose > 0) {
            if ($timenow > $ddtaquiz->timeclose) {
                $dates[] = get_string('ddtaquizclosed', 'ddtaquiz', userdate($ddtaquiz->timeclose));
            } else {
                $dates[] = get_string('ddtaquizcloseson', 'ddtaquiz', userdate($ddtaquiz->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'ddtaquiz');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $ddtaquiz->timeopen) {
            $currentstatus = get_string('ddtaquizisclosedwillopen', 'ddtaquiz',
                    userdate($ddtaquiz->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($ddtaquiz->timeclose && $timenow <= $ddtaquiz->timeclose) {
            $currentstatus = get_string('ddtaquizisopenwillclose', 'ddtaquiz',
                    userdate($ddtaquiz->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($ddtaquiz->timeclose && $timenow > $ddtaquiz->timeclose) {
            $currentstatus = get_string('ddtaquizisclosed', 'ddtaquiz');
        } else {
            $currentstatus = get_string('ddtaquizisopen', 'ddtaquiz');
        }
    ...
    /**
     * Set up this class with the structure for a given ddtaquiz.
     * @param \stdClass $ddtaquiz the ddtaquiz settings.
     */
    public function populate_structure($ddtaquiz) {
        global $DB;
        ...
        // Get ddtaquiz sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('ddtaquiz_sections', array('ddtaquizid' => $ddtaquiz->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the ddtaquiz.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'ddtaquiz');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'ddtaquiz');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_ddtaquiz($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_ddtaquiz($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_ddtaquiz($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('ddtaquiz_slots', 'slot', $slotreorder,
                    array('ddtaquizid' => $this->get_ddtaquizid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('ddtaquiz_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        ddtaquiz_update_section_firstslots($this->get_ddtaquizid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE ddtaquizid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {ddtaquiz_slots} WHERE ddtaquizid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_ddtaquizid(), $this->get_ddtaquizid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {ddtaquiz_slots}
                       SET page = page - 1
                     WHERE ddtaquizid = ?
                       AND page > ?
                    ", array($this->get_ddtaquizid(), $page));
        }
    ...
    /**
     * Refresh page numbering of ddtaquiz slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('ddtaquiz_slots', array('ddtaquizid' => $this->get_ddtaquizid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of ddtaquiz slots and save to the database.
     * @param \stdClass $ddtaquiz the ddtaquiz object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('ddtaquiz_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('ddtaquiz_slots', array('ddtaquizid' => $this->get_ddtaquizid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {ddtaquiz_slots} WHERE ddtaquizid = ?', array($this->get_ddtaquizid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('ddtaquiz_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('ddtaquiz_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('ddtaquiz_slots', 'slot', $i - 1,
                    array('ddtaquizid' => $this->get_ddtaquizid(), 'slot' => $i));
        }
        ...
        ddtaquiz_update_section_firstslots($this->get_ddtaquizid(), -1, $slotnumber);
     ...
     *
     * Saves changes to the question grades in the ddtaquiz_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the ddtaquiz table.
     *
     * @param \stdClass $slot row from the ddtaquiz_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('ddtaquiz_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_ddtaquiz($slot->ddtaquizid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('ddtaquiz_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the ddtaquiz_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $ddtaquizslots = $DB->get_records('ddtaquiz_slots', array('ddtaquizid' => $this->get_ddtaquizid()), 'slot');
        $repaginate = new \mod_ddtaquiz\repaginate($this->get_ddtaquizid(), $ddtaquizslots);
        $repaginate->repaginate_slots($ddtaquizslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'ddtaquiz');
        }
        $section->ddtaquizid = $this->get_ddtaquizid();
        $slotsonpage = $DB->get_records('ddtaquiz_slots', array('ddtaquizid' => $this->get_ddtaquizid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('ddtaquiz_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('ddtaquiz_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('ddtaquiz_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('ddtaquiz_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a ddtaquiz.');
        }
        $DB->delete_records('ddtaquiz_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = ddtaquiz_retrieve_tags_for_slot_ids($slotids);
    }
