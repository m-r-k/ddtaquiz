...
/**
 * Renderer outputting the ddtaquiz editing UI.
 *
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \ddtaquiz $ddtaquizobj object containing all the ddtaquiz settings information.
     * @param structure $structure object containing the structure of the ddtaquiz.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\ddtaquiz $ddtaquizobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingddtaquizx', 'ddtaquiz',
                format_string($ddtaquizobj->get_ddtaquiz_name())), 'editingddtaquiz', 'ddtaquiz', '',
                get_string('basicideasofddtaquiz', 'ddtaquiz'), 2);

        // Information at the top.
        $output .= $this->ddtaquiz_state_warnings($structure);

        $output .= html_writer::start_div('mod_ddtaquiz-edit-top-controls');
        $output .= $this->ddtaquiz_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);

        $output .= html_writer::start_div('mod_ddtaquiz-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= $this->total_marks($ddtaquizobj->get_ddtaquiz());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_ddtaquiz/ddtaquizquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_ddtaquiz/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the ddtaquiz,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the ddtaquiz structure.
     * @return string HTML to output.
     */
    public function ddtaquiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function ddtaquiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'ddtaquiz', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'ddtaquizopeningstatus',
    ...
    /**
     * Render the form for setting a ddtaquiz' overall grade
     *
     * @param structure $structure the ddtaquiz structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'ddtaquizsavegradesform form-inline'));
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_ddtaquiz_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary m-l-1',
                'name' => 'savechanges', 'value' => get_string('save', 'ddtaquiz')));
     ...
     * @param structure $structure the structure of the ddtaquiz being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'ddtaquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'ddtaquiz'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->yui_module('moodle-mod_ddtaquiz-repaginate', 'M.mod_ddtaquiz.repaginate.init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'ddtaquiz'),
            'class' => 'btn btn-secondary m-b-1'
        );
        if (!$structure->can_be_edited()) {
            $buttonoptions['disabled'] = 'disabled';
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'ddtaquiz'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_ddtaquiz'),
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_ddtaquiz'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            'class' => 'btn-toolbar',
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'ddtaquiz'),
        );
        ...
        $output .= html_writer::tag('div',
                html_writer::tag('div',
                        html_writer::link('#', get_string('selectall', 'ddtaquiz'), $buttonselectalloptions) .
                        html_writer::tag('span', "/", ['class' => 'separator']) .
                        html_writer::link('#', get_string('selectnone', 'ddtaquiz'), $buttondeselectalloptions),
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'ddtaquiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'ddtaquiz', $select) .
    ...
    /**
     * Render the total marks available for the ddtaquiz.
     *
     * @param \stdClass $ddtaquiz the ddtaquiz settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($ddtaquiz) {
        $totalmark = html_writer::span(ddtaquiz_format_grade($ddtaquiz, $ddtaquiz->sumgrades), 'mod_ddtaquiz_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'ddtaquiz', $totalmark),
     ...
     *
     * @param structure $structure the structure of the ddtaquiz being edited.
     * @param \stdClass $section The ddtaquiz_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'ddtaquiz', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the ddtaquiz.
     * @param \stdClass $section data from the ddtaquiz_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'ddtaquiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'ddtaquiz'),
    ...
    /**
     * Render an icon to remove a section from the ddtaquiz.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'ddtaquiz', $section->heading);
        $url = new \moodle_url('/mod/ddtaquiz/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the ddtaquiz.
     *
     * This function calls {@link core_course_renderer::ddtaquiz_section_question()}
     *
     * @param structure $structure object containing the structure of the ddtaquiz.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_ddtaquiz($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-ddtaquiz-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'ddtaquiz'), array('class' => 'add-menu'));
        ...
        // Disable the link if ddtaquiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'ddtaquiz');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the ddtaquiz.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'ddtaquiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'ddtaquiz');
        }
        ...
        $returnurl = new \moodle_url('/mod/ddtaquiz/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/ddtaquiz/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'ddtaquiz', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'ddtaquiz');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the ddtaquiz.
     *
     * @param structure $structure object containing the structure of the ddtaquiz.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_ddtaquiz(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $ddtaquiz the ddtaquiz settings from the database.
     * @param \stdClass $question data from the question and ddtaquiz_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($ddtaquiz, $question, $label = null, $variant = null) {
        $url = ddtaquiz_question_preview_url($ddtaquiz, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'ddtaquiz');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'ddtaquiz');
    ...
    /**
     * Render an icon to remove a question from the ddtaquiz.
     *
     * @param structure $structure object containing the structure of the ddtaquiz.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the ddtaquiz.
     *
     * @param structure $structure object containing the structure of the ddtaquiz.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('ddtaquizid' => $structure->get_ddtaquizid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'ddtaquiz');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'ddtaquiz');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'ddtaquiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'ddtaquiz'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'ddtaquiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'ddtaquiz'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a ddtaquiz edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = ddtaquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'ddtaquiz').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/ddtaquiz/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = ddtaquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'ddtaquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'ddtaquiz'), array('class' => 'mod_ddtaquiz_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'ddtaquiz')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'ddtaquiz'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_ddtaquiz/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_ddtaquiz\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/ddtaquiz/edit_rest.php';
        $config->sectionurl = '/mod/ddtaquiz/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_ddtaquiz-toolboxes',
                'M.mod_ddtaquiz.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'ddtaquizid' => $structure->get_ddtaquizid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'ddtaquiz');
        $this->page->requires->yui_module('moodle-mod_ddtaquiz-toolboxes',
                'M.mod_ddtaquiz.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'ddtaquizid' => $structure->get_ddtaquizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_ddtaquiz-dragdrop', 'M.mod_ddtaquiz.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'ddtaquizid' => $structure->get_ddtaquizid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_ddtaquiz-dragdrop', 'M.mod_ddtaquiz.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'ddtaquizid' => $structure->get_ddtaquizid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'ddtaquiz', '');
