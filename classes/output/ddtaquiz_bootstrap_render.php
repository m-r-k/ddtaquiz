...
class ddtaquiz_bootstrap_render extends \html_writer
{
    public static function createModal($title, $body, $yesButton, $attr): string
    {
        ...
        if(array_key_exists('size',$attr))
            $size=$attr['size'];
        if(array_key_exists('$position',$attr))
            $size=$attr['$position'];
        ...
        $output .= self::start_div('modal-dialog'.' '.$size.' '.$position, ['role' => 'document']);
        ...
        if (array_key_exists('one-button',$attr) && $attr['one-button']) {
            $output .= $yesButton;
        } else {
            $output .= $yesButton;
    ...
    public static function createDomainCheckboxes($qid, $domains): string
    {
        ...
        $output = self::start_div("domain-content flex-wrap");
