...
/**
 * Defines the ddtaquiz repaginate class.
 *
 ...
 *
 * The ddtaquiz setting allows users to write ddtaquizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the ddtaquiz being manipulated. */
    private $ddtaquizid;
    /** @var array the ddtaquiz_slots for that ddtaquiz. */
    private $slots;

    /**
     * Constructor.
     * @param int $ddtaquizid the id of the ddtaquiz being manipulated.
     * @param stdClass[] $slots the ddtaquiz_slots for that ddtaquiz.
     */
    public function __construct($ddtaquizid = 0, $slots = null) {
        global $DB;
        $this->ddtaquizid = $ddtaquizid;
        if (!$this->ddtaquizid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('ddtaquiz_slots', array('ddtaquizid' => $this->ddtaquizid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('ddtaquiz_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $ddtaquizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($ddtaquizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$ddtaquizslots) {
            return null;
        }
        $newslots = array();
        foreach ($ddtaquizslots as $slot) {
            if ($type == self::LINK) {
