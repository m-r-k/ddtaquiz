...
/**
 * Defines the custom question bank view used on the Edit ddtaquiz page.
 *
...
/**
 * Subclass to customise the view of the question bank for the ddtaquiz editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the ddtaquiz this is used by has been attemptd. */
    protected $ddtaquizhasattempts = false;
    /** @var \stdClass the ddtaquiz settings. */
    protected $ddtaquiz = false;
     ...
     * @param \stdClass $ddtaquiz ddtaquiz settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $ddtaquiz) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->ddtaquizquestionbankcolumns)) {
            $ddtaquizquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $ddtaquizquestionbankcolumns = explode(',', $CFG->ddtaquizquestionbankcolumns);
        }

        foreach ($ddtaquizquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_ddtaquiz\\question\\bank\\' . $fullname)) {
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_ddtaquiz\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_ddtaquiz\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_ddtaquiz\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the ddtaquiz has been attempted,
     * hence whether some bits of UI, like the add this question to the ddtaquiz icon,
     * should be displayed.
     * @param bool $ddtaquizhasattempts whether the ddtaquiz has attempts.
     */
    public function set_ddtaquiz_has_attempts($ddtaquizhasattempts) {
        $this->ddtaquizhasattempts = $ddtaquizhasattempts;
        if ($ddtaquizhasattempts && isset($this->visiblecolumns['addtoddtaquizaction'])) {
            unset($this->visiblecolumns['addtoddtaquizaction']);
        }
    }

    public function preview_question_url($question) {
        return ddtaquiz_question_preview_url($this->ddtaquiz, $question);
    }

    public function add_to_ddtaquiz_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/ddtaquiz/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->ddtaquizhasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the ddtaquiz.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'class' => 'btn btn-primary',
                    'value' => get_string('addselectedquestionstoddtaquiz', 'ddtaquiz'),
            );
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'ddtaquiz');
