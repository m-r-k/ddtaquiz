...
/**
 * This script controls the display of the ddtaquiz reports.
 *
...
$reportlist = ddtaquiz_report_list($context);
if (empty($reportlist)) {
    print_error('erroraccessingreport', 'ddtaquiz');
}
...
if (!is_readable("report/$mode/report.php")) {
    print_error('reportnotfound', 'ddtaquiz', '', $mode);
}

// Open the selected ddtaquiz report and display it.
$file = $CFG->dirroot . '/mod/ddtaquiz/report/' . $mode . '/report.php';
if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'ddtaquiz_' . $mode . '_report';
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'ddtaquiz');
}

$report = new $reportclassname();
$report->display($ddtaquiz, $cm, $course);
