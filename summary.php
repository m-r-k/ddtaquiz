...
/**
 * This page prints a summary of a ddtaquiz attempt before it is submitted.
 *
...
$PAGE->set_url('/mod/ddtaquiz/summary.php', array('attempt' => $attemptid));
...
if (!$attemptobj->is_preview_user() && $messages) {
    print_error('attempterror', 'ddtaquiz', $attemptobj->view_url(),
...
if (empty($attemptobj->get_ddtaquiz()->showblocks)) {
    $PAGE->blocks->show_only_fake_blocks();
}

$navbc = $attemptobj->get_navigation_panel($output, 'ddtaquiz_attempt_nav_panel', -1);
$regions = $PAGE->blocks->get_regions();
$PAGE->blocks->add_fake_block($navbc, reset($regions));

$PAGE->navbar->add(get_string('summaryofattempt', 'ddtaquiz'));
