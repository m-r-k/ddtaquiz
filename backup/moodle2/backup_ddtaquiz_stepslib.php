...
class backup_ddtaquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $this->add_subplugin_structure('ddtaquizaccess', $ddtaquiz, true);
        ...
        $this->add_subplugin_structure('ddtaquizaccess', $attempt, true);

        // Build the tree.
        $ddtaquiz->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $ddtaquiz->add_child($sections);
        $sections->add_child($section);

        $ddtaquiz->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $ddtaquiz->add_child($overrides);
        ...
        $qinstance->set_source_table('ddtaquiz_slots',
                array('ddtaquizid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('ddtaquiz_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('ddtaquiz_sections',
                array('ddtaquizid' => backup::VAR_PARENTID));

        $feedback->set_source_table('ddtaquiz_feedback',
                array('ddtaquizid' => backup::VAR_PARENTID));

        // Ddtaquiz overrides to backup are different depending of user info.
        $overrideparams = array('ddtaquiz' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('ddtaquiz_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('ddtaquiz_grades', array('ddtaquiz' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {ddtaquiz_attempts}
                    WHERE ddtaquiz = :ddtaquiz AND preview = 0',
                    array('ddtaquiz' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $ddtaquiz->set_source_alias('attempts', 'attempts_number');
        ...
        $ddtaquiz->annotate_files('mod_ddtaquiz', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_ddtaquiz', 'feedback', 'id');
