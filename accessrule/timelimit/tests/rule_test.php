...
/**
 * Unit tests for the ddtaquizaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/ddtaquiz/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquizaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $ddtaquiz = new stdClass();
        $ddtaquiz->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $ddtaquizobj = new ddtaquiz($ddtaquiz, $cm, null);
        $rule = new ddtaquizaccess_timelimit($ddtaquizobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('ddtaquiztimelimit', 'ddtaquizaccess_timelimit', format_time(3600)));
