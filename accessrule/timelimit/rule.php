...
/**
 * Implementaton of the ddtaquizaccess_timelimit plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquizaccess_timelimit extends ddtaquiz_access_rule_base {

    public static function make(ddtaquiz $ddtaquizobj, $timenow, $canignoretimelimits) {

        if (empty($ddtaquizobj->get_ddtaquiz()->timelimit) || $canignoretimelimits) {
            return null;
        }
    ...
    public function description() {
        return get_string('ddtaquiztimelimit', 'ddtaquizaccess_timelimit',
                format_time($this->ddtaquiz->timelimit));
    }

    public function end_time($attempt) {
        return $attempt->timestart + $this->ddtaquiz->timelimit;
    }
    ...
    public function add_preflight_check_form_fields(mod_ddtaquiz_preflight_check_form $ddtaquizform,
        ...
        $mform->addElement('header', 'honestycheckheader',
                get_string('confirmstartheader', 'ddtaquizaccess_timelimit'));
        $mform->addElement('static', 'honestycheckmessage', '',
                get_string('confirmstart', 'ddtaquizaccess_timelimit', format_time($this->ddtaquiz->timelimit)));
    }
}
