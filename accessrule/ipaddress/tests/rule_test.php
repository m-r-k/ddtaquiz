...
/**
 * Unit tests for the ddtaquizaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/ddtaquiz/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquizaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $ddtaquiz->subnet = getremoteaddr(null);
        if (!empty($ddtaquiz->subnet)) {
            $ddtaquizobj = new ddtaquiz($ddtaquiz, $cm, null);
            $rule = new ddtaquizaccess_ipaddress($ddtaquizobj, 0);
