...
/**
 * Implementaton of the ddtaquizaccess_securewindow plugin.
 *
...
/**
 * A rule for ensuring that the ddtaquiz is opened in a popup, with some JavaScript
 * to prevent copying and pasting, etc.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class ddtaquizaccess_securewindow extends ddtaquiz_access_rule_base {
    ...
    public static function make(ddtaquiz $ddtaquizobj, $timenow, $canignoretimelimits) {
        ...
        if ($ddtaquizobj->get_ddtaquiz()->browsersecurity !== 'securewindow') {
            return null;
        }
    ...
    public function setup_attempt_page($page) {
        ...
        if ($this->ddtaquizobj->is_preview_user()) {
            return;
        }

        $page->add_body_class('ddtaquiz-secure-window');
        $page->requires->js_init_call('M.mod_ddtaquiz.secure_window.init',
                null, false, ddtaquiz_get_js_module());
    }
    ...
    public static function get_browser_security_choices() {
        return array('securewindow' =>
                get_string('popupwithjavascriptsupport', 'ddtaquizaccess_securewindow'));
    }
}
