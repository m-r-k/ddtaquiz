...
// Get the ddtaquiz slot.
$slot = $DB->get_record('ddtaquiz_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_ddtaquiz');
}

if (!$ddtaquiz = $DB->get_record('ddtaquiz', array('id' => $slot->ddtaquizid))) {
    print_error('invalidddtaquizid', 'ddtaquiz');
}

$cm = get_coursemodule_from_instance('ddtaquiz', $slot->ddtaquizid, $ddtaquiz->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/ddtaquiz/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/ddtaquiz/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_ddtaquiz\form\randomquestion_form(new moodle_url('/mod/ddtaquiz/editrandom.php'),
...
$currentslottags = ddtaquiz_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the ddtaquiz_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the ddtaquiz_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('ddtaquiz_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_ddtaquiz');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_ddtaquiz');
