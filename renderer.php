...
class mod_ddtaquiz_renderer extends plugin_renderer_base {
     ...
     *
     * @param ddtaquiz_attempt $attemptobj an instance of ddtaquiz_attempt.
     ...
     * @param mod_ddtaquiz_display_options $displayoptions instance of mod_ddtaquiz_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(ddtaquiz_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_ddtaquiz_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(ddtaquiz_attempt $attemptobj, $slot, $seq,
            mod_ddtaquiz_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(ddtaquiz_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_ddtaquiz_name(), true,
                                  array("context" => $attemptobj->get_ddtaquizobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox ddtaquizreviewsummary'));
     ...
     *
     * @param ddtaquiz_attempt $attemptobj instance of ddtaquiz_attempt
     ...
     * @param mod_ddtaquiz_display_options $displayoptions instance of mod_ddtaquiz_display_options
     */
    public function questions(ddtaquiz_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_ddtaquiz_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_ddtaquiz.init_review_form', null, false,
                ddtaquiz_get_js_module());
     ...
     * @param ddtaquiz_attempt $attemptobj instance of ddtaquiz_attempt
     */
    public function finish_review_link(ddtaquiz_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_ddtaquiz.secure_window.init_close_button',
                    array($url), false, ddtaquiz_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'ddtaquiz'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_ddtaquiz-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'ddtaquiz'),
                    array('class' => 'mod_ddtaquiz-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(ddtaquiz_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'ddtaquiz'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_ddtaquiz-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'ddtaquiz'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_ddtaquiz-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the ddtaquiz timer.
     * @return string HTML content.
     */
    public function countdown_timer(ddtaquiz_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the ddtaquiz to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'ddtaquiz') . ' ' .
                html_writer::tag('span', '', array('id' => 'ddtaquiz-time-left')),
                array('id' => 'ddtaquiz-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'ddtaquiz'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param ddtaquiz_nav_panel_base $panel instance of ddtaquiz_nav_panel_base
     */
    public function navigation_panel(ddtaquiz_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_ddtaquiz.nav.init', null, false,
                ddtaquiz_get_js_module());
    ...
    /**
     * Display a ddtaquiz navigation button.
     *
     * @param ddtaquiz_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_ddtaquiz_nav_question_button(ddtaquiz_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'ddtaquiz');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'ddtaquiz', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-ddtaquiz-page' => $button->page);
    ...
    /**
     * Display a ddtaquiz navigation heading.
     *
     * @param ddtaquiz_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_ddtaquiz_nav_section_heading(ddtaquiz_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_ddtaquiz-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_ddtaquiz_links_to_other_attempts $links
     */
    protected function render_mod_ddtaquiz_links_to_other_attempts(
            mod_ddtaquiz_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(ddtaquiz $ddtaquizobj, mod_ddtaquiz_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($ddtaquizobj->get_ddtaquiz_name(), true,
                                  array("context" => $ddtaquizobj->get_context())));
        $output .= $this->ddtaquiz_intro($ddtaquizobj->get_ddtaquiz(), $ddtaquizobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param ddtaquiz_attempt $attemptobj Instance of ddtaquiz_attempt
     * @param int $page Current page number
     * @param ddtaquiz_access_manager $accessmanager Instance of ddtaquiz_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->ddtaquiz_notices($messages);
     ...
     */
    public function ddtaquiz_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'ddtaquiz'), 3) .
                $this->access_messages($messages), 'ddtaquizaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param ddtaquiz_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_ddtaquiz()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the ddtaquiz?
     * @param string $navmethod Optional ddtaquiz attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'ddtaquiz'), 'class' => 'mod_ddtaquiz-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'ddtaquiz');
        } else {
            $nextlabel = get_string('navigatenext', 'ddtaquiz');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_ddtaquiz-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'ddtaquiz'), 'class' => 'mod_ddtaquiz-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_ddtaquiz.timer.init', $options, false, ddtaquiz_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'ddtaquiz'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'ddtaquiz'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_ddtaquiz.secure_window.close',
                array($url, $delay), false, ddtaquiz_get_js_module());
     ...
     *
     * @param ddtaquiz_attempt $attemptobj
     * @param mod_ddtaquiz_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_ddtaquiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'ddtaquiz'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable ddtaquizsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'ddtaquiz'), get_string('status', 'ddtaquiz'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'ddtaquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'ddtaquizsummaryheading';
            }
            ...
            if ($attemptobj->is_question_flagged($slot)) {
                // Ddtaquiz has custom JS manipulating these image tags - so we can't use the pix_icon method here.
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'ddtaquizsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == ddtaquiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'ddtaquiz'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'ddtaquiz'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == ddtaquiz_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'ddtaquiz'), null,
                    get_string('submitallandfinish', 'ddtaquiz')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == ddtaquiz_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'ddtaquiz', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'ddtaquiz', userdate($duedate));
        }
     ...
     * @param array $ddtaquiz Array conting ddtaquiz data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this ddtaquiz
     * @param mod_ddtaquiz_view_object $viewobj
     ...
     *      attempt this ddtaquiz now, if appicable this ddtaquiz
     */
    public function view_page($course, $ddtaquiz, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($ddtaquiz, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($ddtaquiz, $context, $viewobj);
        $output .= $this->view_result_info($ddtaquiz, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'ddtaquizattempt');
        return $output;
    }
     ...
     * @param mod_ddtaquiz_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_ddtaquiz_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'ddtaquiz'), 'get',
     ...
     * @param mod_ddtaquiz_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_ddtaquiz_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_ddtaquiz_preflight_check_form from ' .
                    'ddtaquiz_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' ddtaquizstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' ddtaquizsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_ddtaquiz/preflightcheck', 'init',
                array('.ddtaquizstartbuttondiv [type=submit]', get_string('startattempt', 'ddtaquiz'),
                       '#mod_ddtaquiz_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'ddtaquiz'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editddtaquiz', 'ddtaquiz'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the ddtaquiz
     *
     * @param int $course The course ID
     * @param array $ddtaquiz Array contingin ddtaquiz data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $ddtaquiz, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($ddtaquiz, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'ddtaquiz'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $ddtaquiz, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($ddtaquiz, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'ddtaquiz'));
     ...
     * @return string HTML to output.
     */
    public function view_information($ddtaquiz, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print ddtaquiz name and description.
        $output .= $this->heading(format_string($ddtaquiz->name));
        $output .= $this->ddtaquiz_intro($ddtaquiz, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'ddtaquizinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/ddtaquiz:viewreports', $context)) {
            if ($strattemptnum = $this->ddtaquiz_attempt_summary_link_to_reports($ddtaquiz, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'ddtaquizattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the ddtaquiz intro.
     * @param object $ddtaquiz the ddtaquiz settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function ddtaquiz_intro($ddtaquiz, $cm) {
        if (html_is_blank($ddtaquiz->intro)) {
            return '';
        }

        return $this->box(format_module_intro('ddtaquiz', $ddtaquiz, $cm->id), 'generalbox', 'intro');
    }
     ...
     *
     * @param array $ddtaquiz Array contining ddtaquiz data
     * @param int $context The page context ID
     * @param mod_ddtaquiz_view_object $viewobj
     */
    public function view_table($ddtaquiz, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable ddtaquizattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        ...
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'ddtaquiz') . ' / ' .
                    ddtaquiz_format_grade($ddtaquiz, $ddtaquiz->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    ddtaquiz_format_grade($ddtaquiz, $ddtaquiz->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        ...
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'ddtaquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = ddtaquiz_format_grade($ddtaquiz, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = ddtaquiz_rescale_grade($attemptobj->get_sum_marks(), $ddtaquiz, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == ddtaquiz_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $ddtaquiz->grademethod == DDTAQUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = ddtaquiz_format_grade($ddtaquiz, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = ddtaquiz_feedback_for_grade($attemptgrade, $ddtaquiz, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param ddtaquiz_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case ddtaquiz_attempt::OVERDUE:
                return get_string('stateoverdue', 'ddtaquiz') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'ddtaquiz',
    ...
    /**
     * Generates data pertaining to ddtaquiz results
     *
     * @param array $ddtaquiz Array containing ddtaquiz data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_ddtaquiz_view_object $viewobj
     */
    public function view_result_info($ddtaquiz, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = ddtaquiz_get_grading_option_name($ddtaquiz->grademethod);
                $a->mygrade = ddtaquiz_format_grade($ddtaquiz, $viewobj->mygrade);
                $a->ddtaquizgrade = ddtaquiz_format_grade($ddtaquiz, $ddtaquiz->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'ddtaquiz', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = ddtaquiz_format_grade($ddtaquiz, $viewobj->mygrade);
                $a->maxgrade = ddtaquiz_format_grade($ddtaquiz, $ddtaquiz->grade);
                $a = get_string('outofshort', 'ddtaquiz', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'ddtaquiz', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'ddtaquiz'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'ddtaquizteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'ddtaquiz'), 3);
            $resultinfo .= html_writer::div(
                    ddtaquiz_feedback_for_grade($viewobj->mygrade, $ddtaquiz, $context),
                    'ddtaquizgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'ddtaquiz'));
            $button->add_action(new popup_action('click', $url, 'ddtaquizpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'ddtaquiz'),
     ...
     */
    public function ddtaquiz_attempt_summary_link_to_reports($ddtaquiz, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'ddtaquiz'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'ddtaquiz'), FORMAT_MARKDOWN, $options);
...
class mod_ddtaquiz_links_to_other_attempts implements renderable {
    /**
...
class mod_ddtaquiz_view_object {
    /** @var array $infomessages of messages with information to display about the ddtaquiz. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this ddtaquiz. */
    public $attempts;
    /** @var array $attemptobjs ddtaquiz_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var ddtaquiz_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the ddtaquiz. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this ddtaquiz. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a ddtaquiz. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $ddtaquizhasquestions whether the ddtaquiz has any questions. */
    public $ddtaquizhasquestions;
